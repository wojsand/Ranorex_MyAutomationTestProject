...
<div class="form-horizontal">
	<fieldset>
		<legend>
			<itera:message key="graphicalExport.landscape.content"/>
		</legend>
		...
		<div class="control-group">
			<form:select path="graphicalOptions.selectedBbType" id="contentElementType" onchange="flowAction('selectContentType');" disabled="${memBean.graphicalOptions.dialogStep != 1}">
				<form:option value="">
					<itera:message key="graphicalExport.landscape.select.content" />
				</form:option>
			...
			<a href="#" rel="tooltip" data-original-title="Hotkey: <img/> <itera:message key="button.changeContent.tooltip"/>">
				&nbsp;<input type="button" class="link btn" id="changeContentButton" class="link" <c:out value="${changeContent}" escapeXml="false"/>
					onclick="flowAction('changeContentType');" value="<itera:message key="button.edit"/>"/>
			</a>
		</div>
		...
		<c:if test="${memBean.graphicalOptions.dialogStep > 1}">
			...
			<div class="control-group">
				<a href="#" rel="tooltip" data-original-title="Hotkey: <img/> <itera:message key="button.filterContent.tooltip"/>">
					<input type="button" class="link btn" id="filterContentButton" class="link" value="<itera:message key="button.filter"/>" onclick="flowAction('filterContentResults')" style="filterContentElements"/>
				</a>
			</div>
		</c:if>
		...
		<c:if test="${ memBean.graphicalOptions.levels.contentTopLevel > 1 || memBean.graphicalOptions.levels.contentBottomLevel > 1 }">
			<div class="control-group">
				<b class="help-block">
					...
					<itera:message key="graphicalExport.landscape.configuration.content.levels">
						<itera:param value="${contentType}"/>
						<itera:param value="${memBean.graphicalOptions.levels.contentTopLevel}"/>
						<itera:param value="${memBean.graphicalOptions.levels.contentBottomLevel}"/>
					</itera:message>
				</b>
