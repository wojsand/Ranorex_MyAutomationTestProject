...
<div class="form-horizontal">
	<fieldset>
		<legend>
			<itera:message key="graphicalExport.landscape.rowAssociation"/>
		</legend>
		...
		<c:if test="${memBean.graphicalOptions.dialogStep > 1}">
			...
			<c:if test="${!isAttributeSet}">
				<div class="control-group">
					<label class="control-label" style="padding-top: 10px; margin-bottom: 0pt; width:60px">
						<itera:message key="graphicalExport.landscape.relation"/>:
					</label>
					...
					<div class="controls">
						<form:select path="graphicalOptions.selectedRowRelation" disabled="${isRowRelationSet}"
							...
							<form:option value="">
								<itera:message key="reports_export_select_relation" />
							</form:option>
							<c:forEach items="${memBean.graphicalOptions.availableRelations}" var="relation">
								<form:option value="${relation.name}">
									<itera:message key="${relation.nameKeyForPresentation}" />
								</form:option>
							</c:forEach>
						</form:select>
						
						<c:if test="${isAttributeSet || isRowRelationSet}">
			     			&nbsp;<input type="button" class="link btn" onclick="flowAction('changeRowType')" value="<itera:message key="button.edit"/>" />
			      		</c:if>
					</div>
				</div>
			</c:if>
			...
			<c:if test="${!(isAttributeSet || isRowRelationSet)}">
				<b class="help-block"><itera:message key="reports.or"/></b>
			</c:if>
			...
			<c:if test="${!isRowRelationSet}">
				<div class="control-group">
					<label class="control-label" style="padding-top: 10px; margin-bottom: 0pt;  width:60px	">
							<itera:message key="graphicalExport.landscape.attribute"/>:
					</label>
			  		...
			  		<div class="controls">
						<form:select path="graphicalOptions.selectedRowAttributeId" disabled="${isAttributeSet}"
						  ...
						  <form:option value="-1">
						  	<itera:message key="graphicalExport.landscape.select.attribute"/>
						  </form:option>
						  <c:forEach items="${memBean.queryResults.contentQuery.queryForms[0].availableAttributes}" var="attribute">
						    ...
						    <c:if test="${attribute.type == 'userdefResponsibility'}">
						     ...
						     <c:if test="${attribute.id == 0}">
						      <form:option value="${attribute.id}">
						        <itera:message key="${attribute.name}"/>
						      </form:option>
						    </c:if>
						  </c:forEach>                     
						</form:select>
		...
		<div class="control-group">
			<c:choose>
			  <c:when test="${memBean.graphicalOptions.selectedRowOption == 1 && isRowRelationSet}">
			    <c:set var="elementType" scope="page">                                
			      <itera:message key="${memBean.graphicalOptions.currentRowRelation.requestedType.typeNamePluralPresentationKey}"/>
			    </c:set>        
			...
			<c:if test="${isRowRelationSet}">
				<div class="control-group">
					<input type="button" class="link btn" onclick="flowAction('filterRowResults');" value="<itera:message key="button.filter"/>" />
				</div>
			</c:if>
		</div>
		...
		<c:if test="${isRowRelationSet && ( memBean.graphicalOptions.levels.sideAxisTopLevel > 1 || memBean.graphicalOptions.levels.sideAxisBottomLevel > 1 )}">
			<div class="control-group">
				<b class="help-block">
					...
					<itera:message key="graphicalExport.landscape.configuration.axis.levels">
						<itera:param value="${rowType}"/>
						<itera:param value="${rowAssociationString}"/>
						<itera:param value="${memBean.graphicalOptions.levels.sideAxisTopLevel}"/>
						<itera:param value="${memBean.graphicalOptions.levels.sideAxisBottomLevel}"/>
					</itera:message>
				</b>
		...
		<div class="control-group">
			<label class="checkbox">
		  		...
		  		<itera:message key="graphicalExport.landscape.hideEmptyRows" />
		  	</label>
		</div>
	</fieldset>
</div>
