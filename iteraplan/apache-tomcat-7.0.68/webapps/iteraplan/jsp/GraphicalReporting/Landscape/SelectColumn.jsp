...
<div class="form-horizontal">
	<fieldset>
		<legend>
			<itera:message key="graphicalExport.landscape.columnAssociation"/>
		</legend>
				...
				<div class="control-group">
					...
					<div class="controls">
						...
						<c:if test="${isAttributeSet || isColumnRelationSet}">
							&nbsp;<input type="button" class="link btn" onclick="flowAction('changeColumnType');" value="<itera:message key="button.edit"/>" />
						</c:if>
					</div>
				</div>
			</c:if>
		...
		<div class="control-group">
			<c:choose>
				<c:when test="${memBean.graphicalOptions.selectedColumnOption == 1 && isColumnRelationSet}">
					<c:set var="elementType" scope="page">                                
						<itera:message key="${memBean.graphicalOptions.currentColumnRelation.requestedType.typeNamePluralPresentationKey}"/>
					</c:set>
			...
			<c:if test="${isColumnRelationSet}">
				<input type="button" class="link btn" onclick="flowAction('filterColumnResults');" value="<itera:message key="button.filter"/>" />
			</c:if>
		</div>
			...
			<div class="control-group">
				<b class="help-block">
					...
					<itera:message key="graphicalExport.landscape.configuration.axis.levels">
						<itera:param value="${columnType}"/>
						<itera:param value="${columnAssociationString}"/>
						<itera:param value="${memBean.graphicalOptions.levels.topAxisTopLevel}"/>
						<itera:param value="${memBean.graphicalOptions.levels.topAxisBottomLevel}"/>
					</itera:message>
				</b>
		...
		<div class="control-group">
			<label class="checkbox">
				...
				<itera:message key="graphicalExport.landscape.hideEmptyColumns" />
			</label>
		</div>
