...
<c:if test="${permissionCreateReports == 'true'}" >
	<p class="aligned"><itera:message key="graphicalExport.portfolio.helpContentType" /></p>
	
	<form:select path="graphicalOptions.selectedBbType" onchange="flowAction('selectPortfolioType');">
	
	  <itera:define id="available_array" name="memBean" property="graphicalOptions.availableBbTypes"/>
	  <c:forEach items="${available_array}" var="available" varStatus="countStatus">	
	...
	<p class="aligned">
		<itera:message key="graphicalExport.portfolio.helpStep1">
	...
	<tiles:insertTemplate template="/jsp/GraphicalReporting/GraphicalReportingQueryForm.jsp" flush="true">
		<tiles:putAttribute name="hasPermission" value="${not empty available_array}"/>
		<tiles:putAttribute name="flowAction" value="goToPortfolioExportStep2"/>
	</tiles:insertTemplate>
</c:if>
