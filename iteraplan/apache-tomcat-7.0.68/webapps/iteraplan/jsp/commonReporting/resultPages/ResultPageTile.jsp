...
<tilesx:useAttribute name="showAddColumnButton" ignore="true" />
<tilesx:useAttribute name="show_nettoexport" ignore="true" />
...
<div id="ReportingResultTableModule" class="row-fluid module">
	<div class="module-heading">
		<itera:message key="reports.results" />:&nbsp;<c:out value="${resultSize}" />
        ...
        <div class="btn-group pull-right">
          <div class="btn dropdown-toggle" data-toggle="dropdown" style="font-weight: normal; margin:-5px -5px 0 10px;">
            ...
            <span><itera:message key="reports.nettoexport.button"></itera:message></span>
					            ...
					            <c:forEach items="${visibleColumns}" var="column" varStatus="s">
									...
									<c:choose>
										...
										<c:when test="${type=='date'}">
											...
											<itera:message var="dateFormat" key="calendar.dateFormat"/>
										...
										<c:when test="${type=='typeofstatus'}">
											...
											<c:set var="content">
												<itera:message key="${statusString}" />
											</c:set>
										</c:when>
						                        ...
						                        <c:forEach var="attrValAss" items="${entity.attributeValueAssignments}">
						                        	<c:if test="${attrValAss.attributeValue.attributeType.id == column.field}">
							                            ...
							                            <c:set var="assignedValuesPerAttributeCounter" value="${assignedValuesPerAttributeCounter + 1}"/>
							                                	<c:out value="${attrValAss.attributeValue.localizedValueString}" escapeXml="true"  />
						                          	</c:if>
						                        </c:forEach>
					                      	</c:set>
					                    </c:when>
														...
														<c:choose>
															<c:when test="${sealState == 'seal.valid'}">
																<li id="sealBarValidSeal">
																	<span class="ToolText" onmouseover="this.className='ToolTextHover';" onMouseOut="this.className='ToolText';" >
																		<input type="button"/>
																		<span><itera:message key="seal"/>: <itera:message key="seal.valid"/></span>
																	</span>
																</li>
															</c:when>
															<c:when test="${sealState == 'seal.outdated'}">
																<li id="sealBarOutdatedSeal">
																	<span class="ToolText" onmouseover="this.className='ToolTextHover';" onMouseOut="this.className='ToolText';" >
																		<input type="button"/>
																		<span><itera:message key="seal"/>: <itera:message key="seal.outdated"/></span>
																	</span>
																</li>
															</c:when>
															<c:when test="${sealState == 'seal.invalid'}">
																<li id="sealBarInvalidSeal">
																	<span class="ToolText" onmouseover="this.className='ToolTextHover';" onMouseOut="this.className='ToolText';" >
																		<input type="button"/>
																		<span><itera:message key="seal"/>: <itera:message key="seal.invalid"/></span>
																	</span>
																</li>
															</c:when>
															<c:when test="${sealState == 'seal.notavailable'}">
																<li id="sealBarNotavailableSeal">
																	<span class="ToolText" onmouseover="this.className='ToolTextHover';" onMouseOut="this.className='ToolText';" >
																		<input type="button" />
																		<span><itera:message key="seal"/>: <itera:message key="seal.notavailable"/></span>
																	</span>
																</li>
															</c:when>
														</c:choose>
