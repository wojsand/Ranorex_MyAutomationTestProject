...
<tilesx:useAttribute name="hideHeader" ignore="true" />
...
<div id="PropertiesFormContainer" class="row-fluid module">
	<c:if test="${empty hideHeader}">
		<div class="module-heading">
		    <itera:message key="reports.resultAttributes" /> 
		    <itera:message key="${memBean.reportResultType.typeNamePluralPresentationKey}" />
		    <c:if test="${massUpdateMode == true}">
		    	<c:out value=" "/>
		        <itera:message key="global.for" />
		        <c:out value=" "/>
...
<div class="row-fluid module">
	<div class="row-fluid">
		<div class="module-heading">
				<itera:message key="reports.timeseries.heading" />
		</div>
		<div class="module-body">
			<div class="row-fluid">
				<div class="row-fluid">
					<c:forEach var="quantor" items="${memBean.queryResult.timeseriesQuery.quantors}">
						<label class="radio inline"><form:radiobutton path="queryResult.timeseriesQuery.quantor" name="timeseries_quantor" value="${quantor}" /><itera:message key="${quantor.name}" /></label>
					</c:forEach>
				<itera:message key="reports.timeseries.value_s"/>&nbsp;<itera:message key="global.between"/>
				&nbsp;
				...
				<a id="<c:out value="dateRemoveAnchor2_ts"/>" class="link"
					...
					onclick="$('#<c:out value="endDateAsString_ts"/>').val('');" >
				   	<i class="icon-remove"></i>
				</a> <itera:message key="reports.timeseries.must_meet_criterion"/>
				</div>
