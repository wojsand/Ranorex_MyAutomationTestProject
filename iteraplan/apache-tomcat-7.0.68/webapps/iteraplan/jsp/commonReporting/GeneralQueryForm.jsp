	...
	<div class="ReportingRequestButtons">
		...
		<c:choose>
      		<c:when test="${memBean.tabularOptions.resultFormat eq 'reports_exportHTML' or empty memBean.tabularOptions.resultFormat}">
    			...
    			<input id="sendQueryButton" class="link btn btn-primary" onclick="flowAction('requestReport')" type="button" value="<itera:message key="button.sendQuery" />"/>
      		</c:when>
   			 <c:otherwise>
   			 <!-- show the downloadlabel first and then start download -->
    			<input id="sendQueryButton" class="link btn btn-primary" onclick="flowAction('triggerDownloadEvent')" type="button" value="<itera:message key="button.sendQuery" />"/>
    		</c:otherwise>
		</c:choose>
		...
		<a href="#" rel="tooltip" data-original-title="Hotkey: <img/> <itera:message key="button.resetReport.tooltip"/>">
			<input id="buttonResetReport" class="link btn" onclick="flowAction('resetReport')" type="button" value="<itera:message key="button.reset" />"/>
		</a>
	</div>
</c:if>
