...
<tilesx:useAttribute name="componentModel" />
	...
	<div id="businessMapping_rowColumnSettings" class="well">
    	...
    	<table class="tableInWell">
    		<tbody>
				...
				<tr>
	      			<td>
						<itera:message key="businessMapping.columns"/>:
					</td>
					<td width="5%"/>
		    		<td>
	      				<c:if test="${componentModel.actualColumnPage > 0}">
				    		<a href="#" id="arrow_first_page" rel="tooltip" data-original-title="Hotkey: <img/> <itera:message key='search.firstPage.tooltip'/>" class="link navigateSearchResults" onclick="switchToPage('actualColumnPage', 0);">
					...
					<td>
	      				<c:if test="${componentModel.actualColumnPage > 0}">
				    		<a href="#" id="arrow_prev_page" rel="tooltip" data-original-title="Hotkey: <img/> <itera:message key='search.prevPage.tooltip'/>" class="link navigateSearchResults" onclick="switchPage('actualColumnPage', -1);">
					...
					<td style="text-align: center">
						...
						<itera:message key="search.page"/><c:out value=" "/><fmt:formatNumber value="${componentModel.actualColumnPage + 1}" maxFractionDigits="0"/>/<fmt:formatNumber value="${componentModel.numberOfColumnPages + 1}" maxFractionDigits="0"/>
	      			</td>
		  			<td>
						<c:if test="${componentModel.actualColumnPage < componentModel.numberOfColumnPages}">
							<a href="#" id="arrow_next_page" rel="tooltip" data-original-title="Hotkey: <img/> <itera:message key='search.nextPage.tooltip'/>" class="link navigateSearchResults" onclick="switchPage('actualColumnPage', 1);">
		    		...
		    		<td>
						<c:if test="${componentModel.actualColumnPage < componentModel.numberOfColumnPages}">
			  				<a href="#" id="arrow_last_page" rel="tooltip" data-original-title="Hotkey: <img/> <itera:message key='search.lastPage.tooltip'/>" class="link navigateSearchResults" onclick="switchToPage('actualColumnPage', ${componentModel.numberOfColumnPages});">
				    ...
				    <td style="text-align: right">
						<itera:message key="businessMapping.numberOfColumns"/>:
					</td>
					<td>
					...
					<td width="5%"/>
			      	<td width="20%" rowspan="2">
			      		<input type="button" class="btn" value="<itera:message key="businessMapping.apply"/>" onclick="flowAction('refresh');" />
			      	</td>
				</tr>
				...
				<%-- Settings for Row Page --%>
	      		<tr>
	      			<td>
						<itera:message key="businessMapping.rows"/>:
					</td>
					<td/>
				    <td>
			      		<c:if test="${componentModel.actualRowPage > 0}">
						    <a href="#" id="arrow_first_page" rel="tooltip" data-original-title="Hotkey: <img/> <itera:message key='search.firstPage.tooltip'/>" class="link navigateSearchResults" onclick="switchToPage('actualRowPage', 0);">
						    	<img alt="First Page" src="<c:url value="/images/pfeil_ganzlinks_schwarz.gif" />" title="" />
					        </a>
						</c:if>
				    </td>
					<td>
			     	 	<c:if test="${componentModel.actualRowPage > 0}">
						    <a href="#" id="arrow_prev_page" rel="tooltip" data-original-title="Hotkey: <img/> <itera:message key='search.prevPage.tooltip'/>" class="link navigateSearchResults" onclick="switchPage('actualRowPage', -1);">
						    	<img alt="Previous Page" src="<c:url value="/images/pfeil_einenlinks_schwarz.gif" />" title="" />
					        </a>
						</c:if>
					</td>
					<td style="text-align: center">
				      ...
				      <itera:message key="search.page"/><c:out value=" "/><fmt:formatNumber value="${componentModel.actualRowPage + 1}" maxFractionDigits="0"/>/<fmt:formatNumber value="${componentModel.numberOfRowPages + 1}" maxFractionDigits="0"/>
			      	</td>
				  	<td>
						<c:if test="${componentModel.actualRowPage < componentModel.numberOfRowPages}">
							<a href="#" id="arrow_next_page" rel="tooltip" data-original-title="Hotkey: <img/> <itera:message key='search.nextPage.tooltip'/>" class="link navigateSearchResults" onclick="switchPage('actualRowPage', 1);">
						    	<img alt="Next Page" src="<c:url value="/images/pfeil_einenrechts_schwarz.gif" />" title="" />
					        </a>
						</c:if>
				  	</td>
				    <td>
						<c:if test="${componentModel.actualRowPage < componentModel.numberOfRowPages}">
					  		<a href="#" id="arrow_last_page" rel="tooltip" data-original-title="Hotkey: <img/> <itera:message key='search.lastPage.tooltip'/>" class="link navigateSearchResults" onclick="switchToPage('actualRowPage', ${componentModel.numberOfRowPages});">
						    	<img alt="Last Page" src="<c:url value="/images/pfeil_ganzrechts_schwarz.gif" />" title="" />
					        </a>
						</c:if>
				    </td>
				    <td/>
				    <td style="text-align: right">
						<itera:message key="businessMapping.numberOfRows"/>:
					</td>
					<td>
...
<c:if test="${componentMode == 'EDIT'}">
	<label class="checkbox">
		...
		<itera:message key="businessMapping.enableRowColumnUpdates"/>
	</label>
</c:if>
		...
		<c:if test="${componentMode == 'EDIT' && rowColumnUpdateEnabled}">
			<tr>
				<td class="businessMapping_table_header tableInTable">
					...
					<input id="takeOver_all" type="button" class="btn" value="<itera:message key="businessMapping.takeOverAllValues"/>" onclick="flowAction('rowColumnUpdate');" />
				</td>
				<td class="businessMapping_table_header" />
			  	<c:forEach var="bbColumn" items="${bbForColumn}" varStatus="columnIndex">
			  		<td class="businessMapping_table_header tableInTable">
						...
						<input id="takeOver_column_${columnIndex.index}" type="button" class="btn" value="<itera:message key="businessMapping.takeOverColumnValues"/>" onclick="flowAction('rowColumnUpdate');" />
					</td>
			  	</c:forEach>
			</tr>
		</c:if>
				...
				<%-- Insert column updater ( standard values ) --%>
	  			<c:if test="${componentMode == 'EDIT' && rowColumnUpdateEnabled}">
					<td class="businessMapping_table_header">
						...
						<input id="takeOver_row_${rowIndex.index}" type="button" class="btn" value="<itera:message key="businessMapping.takeOverRowValues"/>" onclick="flowAction('rowColumnUpdate');" />
					</td>
				</c:if>
				<th class="businessMapping_table_header" >
