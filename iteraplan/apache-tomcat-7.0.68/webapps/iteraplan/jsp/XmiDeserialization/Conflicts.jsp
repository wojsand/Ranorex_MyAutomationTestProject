...
<div class="alert alert-error">
	...
	<c:choose>
		<c:when test="${functionalPermissionImport == true}">
			<span class="errorHeader"><itera:message key="global.xmi.import_failed"/><br/></span>
			<%-- TODO: convert table into module --%>
			<table width="99%">
				<tr>
					<td>
						<span class="errorHeader"><itera:message key="global.xmi.import_failed_solve"/></span>
					</td>
					<td align="right">
						<input type="submit" value="<itera:message key="button.xmi.solve_and_import" />"
						onclick="createHiddenField('clickedButton', 'button.xmi.solve_and_import');" />					
					</td>
					<td align="right">
						<input type="submit" value="<itera:message key="button.import.xmi.hide_conflicts" />"
			...
			<thead>
				<tr>
			 		<th colspan="100%"><itera:message key="global.xmi.import_conflicts"/></th>
				 </tr>
			 </thead>
			 <tbody>
				<tr class="subHeader">				
