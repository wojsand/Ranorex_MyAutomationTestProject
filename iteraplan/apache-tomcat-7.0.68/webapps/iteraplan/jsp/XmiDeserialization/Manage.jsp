	...
	<c:when test="${functionalPermissionExport == true}">
		<div id="XMIExportModul" class="row-fluid module">
			<div class="module-heading">
				<itera:message key="xmi.serialization" />
			</div>
			<div class="row-fluid">
									...
									<td class="col-ico">
										<a  id="export_xmi_zip"
											...
											<i class="icon-download-alt"></i>
				 							<itera:message key="button.export.xmi.zip" />
										</a> 
									</td>
									<td><itera:message key="xmi.zip.description" /></td>
								</tr>
								<tr>
									<td>
										<a  id="export_xmi"
											...
											style="width: 212px" >
											<i class="icon-download-alt"></i>
				 							<itera:message key="button.export.xmi" />
										</a> 
									</td>
									<td><itera:message key="xmi.serialization.description" /></td>
								</tr>	
								<tr>
									<td>
										<a  id="export_xmi_extended"
											...
											style="width: 212px" >
											<i class="icon-download-alt"></i>
				 							<itera:message key="button.export.xmi.extended" />
										</a> 
									</td>
									<td><itera:message key="xmi.serialization.extended.description" /></td>
								</tr>
								<tr>
									<td>
										<a  id="export_ecore"
											...
											style="width: 212px" >
											<i class="icon-download-alt"></i>
				 							<itera:message key="button.export.ecore" />
										</a> 
									</td>				
									<td><itera:message key="xmi.ecore.description" /></td>
								</tr>
							</tbody>
						</table>
					</div>
		...
		<div id="XMIExportSpreadsheetModul" class="row-fluid module">
			<div class="module-heading">
				<itera:message key="xmi.exportForTabularReporting" />
			</div>
			<div class="row-fluid">
				<div class="module-body-table">
					<div class="row-fluid">
								...
								<tr>
									<td class="col-ico">
										<a  id="export_ecore_modified"
											...
											<i class="icon-download-alt"></i>
				 							<itera:message key="button.export.ecore.modified" />
										</a> 
									</td>
									<td><itera:message key="xmi.ecore.modified.description" /></td>
								</tr>
							</tbody>
						</table>
					</div>
		...
		<c:choose>
			<c:when test="${functionalPermissionImport == true}">
				<div id="XMIImortModul" class="row-fluid module">
					<div class="module-heading">
						<itera:message key="global.xmi_import" />
					</div>
					<div class="row-fluid">
						<div class="module-body-table">
							<div class="row-fluid">
											...
											<c:choose>
												<c:when test="${(dialogMemory.xmiFileNull == true)}">
													<td>
														<div class="alert">
															...
															<itera:message key="global.xmi.import_description" />
														</div>
													</td>							
												</c:when>
												<c:when test="${(dialogMemory.wrongFileType == true)}">
													<td>
														<div class="alert">
															<a class="close" data-dismiss="alert">×</a>
															<itera:message key="global.xmi.wrong_filetype" />
														</div>
													</td>							
												</c:when>								
												<c:otherwise>
													<td><itera:message key="global.xmi.import_description" /></td>
												</c:otherwise>
											</c:choose>
										</tr>
										<tr>
											<td>
												<input type="submit" style="width: 18em" class="btn"
													value="<itera:message key="button.import.xmi"/>"
				                        ...
				                        <tr>
											<td>
												<input type="submit" style="width: 18em"
													value="<itera:message key="button.import.xmi_restore"/>"
													onclick="createHiddenField('clickedButton', '<c:out value="button.import.xmi_restore"/>');" />
											</td>
											<td><itera:message key="global.xmi.restore_description" /></td>
										</tr>  --%>
									</tbody>
								</table>
							</div>
        		...
        		<c:if test="${dialogMemory.importSuccessful}">
        			<div class="alert alert-success">
	        			<a class="close" data-dismiss="alert">×</a>
	        			<itera:message key="xmi.import.successful" />
        			</div>
        		</c:if>
			</c:when>
			<c:otherwise>
				...
				<span><itera:message key="errors.noXmiDeserializationPermission" /></span></div>
			</c:otherwise>
		</c:choose>
	</c:when>
	<c:otherwise>
