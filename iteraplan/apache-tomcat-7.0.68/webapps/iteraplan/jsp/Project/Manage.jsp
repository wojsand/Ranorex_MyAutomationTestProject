	...
	<c:choose>
		<c:when test="${functionalPermission == true}">
			...
			<tiles:insertTemplate template="/jsp/common/TransactionBar.jsp">
				...
				<tiles:putAttribute name="service_class" value="ProjectService" />
			...
			<tiles:insertTemplate template="/jsp/common/ElementNameComponentView.jsp">
				<tiles:putAttribute name="icon_suffix" value="project" />
			...
			<%-- Insert Table for elementComponent properties --%>
	  		<tiles:insertTemplate template="/jsp/common/RuntimePeriodComponentView.jsp">
	  			<tiles:putAttribute name="path_to_componentModelName" value="componentModel.nameModel"  />
			...
			<itera:tabgroup id="ConfigTabs" maxTabCountPerLine="5" activeTab="${activeTab}">
			  	<itera:tab id="TabHierarchy" 	textKey="global.hierarchy" 		page="/jsp/Project/tabPages/TabHierarchy.jsp" showTab="true" />
			  	<itera:tab id="TabAssignments" 	textKey="button.assignments" 	page="/jsp/Project/tabPages/TabAssignments.jsp" showTab="${showTab}" />
			  	...
			  	<c:if test="${componentMode == 'READ'}">
			  		<itera:tab id="FastExport" 	textKey="global.fastExport" 	page="/jsp/Project/tabPages/TabFastExportProject.jsp" showTab="${showTab}" />
			  	</c:if>
				<c:if test="${componentMode == 'READ' && isHistoryEnabled}">
					<itera:tab id="TabLocalHistory" textKey="global.history"    page="/jsp/Project/tabPages/TabHistory.jsp"/>
				</c:if> 
			</itera:tabgroup>
