...
<c:if test="${!(dialogMemory.errors == null)}">
	<div class="alert alert-error">
		...
		<span class="errorHeader"><itera:message key="errors.header" /><br />
...
<div class="row-fluid">
	<div id="QueryConsoleContainer" class="row-fluid module">
		<div class="module-heading">
		    <itera:message key="global.iteraql" /> 
		</div>
		<div class="row-fluid">
			<div class="module-body">
				<div class="row-fluid">
					<itera:message key="iteraql.query.message"/>
					<br/>
					<textarea name="query" cols="120" rows="3" ><c:out value="${dialogMemory.query}"/></textarea>
					<br/>
					<input id="sendQuery" type="button" class="link btn btn-primary" value="<itera:message key="button.sendQuery" />" onclick="submitForm('sendQuery.do');" />
					<input id="clear" type="button" class="link btn" value="<itera:message key="iteraql.clear.trigger" />" onclick="submitForm('clear.do');"/>
				</div>
			</div>
		</div>
	</div>
    ...
    <div class="accordion-group">
        <div class="accordion-heading">
            <a class="accordion-toggle" data-toggle="collapse" data-parent="#examplesContainer" href="#examples"
              ...
              <itera:message key="iteraql.helpLabel" />
            </a>
        </div>
	    <div id="examples" class="accordion-body collapse">
			<div class="accordion-inner">
				<itera:message key="iteraql.examples" />
				<br/><br/>
				...
				<a href="${onlineDocuLink}/Query+Console+%26+iteraQL" target="_blank" class="iteratecLink"><u><itera:message key="iteraql.userManual" /></u></a>
			</div>
	    </div>
	</div>
</div>
...
<c:choose>
	...
	<itera:message key="interchange.help.choose.destination"/>
		<br/>
  	  <form:select path="selectedDestination" style="width: 320px" >
			<c:forEach items="${dialogMemory.availableDestinations}" var="destination">
				<form:option value="${destination}" >
					<itera:message key="${destination}" />
				</form:option>
			</c:forEach>
		</form:select>
	
		<input id="doInterchange" type="button" class="link btn btn-primary" value="<itera:message key="iteraql.interchange.trigger" />" onclick="createHiddenField('selectedBuildingBlock','${memBean.selectedBuildingBlock}');submitForm('interchange.do');" />
		<br/><br/>
	...
	<c:otherwise>
		<c:if test="${iteraQlBean ne null}">
			<div class="row-fluid">
				<div id="QueryResultsContainer" class="row-fluid module">
					<div class="module-heading">
						<itera:message key="iteraQl.results" />
					</div>
					<div class="row-fluid">
									...
									<c:choose>
										<c:when test="${iteraQlBean.bindingSetResult}">
											...
											<thead>
												<tr>
													<th colspan="6">
														<itera:message key="iteraQl.queryExecTime" />
														<c:out value="${iteraQlBean.execTime}" />
													</th>
												</tr>
												<tr>
													<th colspan="3">
														<itera:message key="iteraQl.resultType" />
												...
												<tr>
													<th colspan="3">
														<itera:message key="iteraQl.resultCount" />
												...
												<tr>
													<th scope="col">
														<b><itera:message key="global.id" /></b>
													</th>
													<th scope="col">
														<b><itera:message key="global.name" /></b>
													</th>
													<th scope="col">
														<b><itera:message key="global.description" /></b>
													</th>
													<th scope="col">
														<b><itera:message key="global.id" /></b>
													</th>
										...
										<c:otherwise>
											...
											<thead>
												<tr>
													<th colspan="3"><itera:message
															key="iteraQl.queryExecTime" />
														<c:out value="${iteraQlBean.execTime}" />
													</th>
												</tr>
												<tr>
													<th colspan="3"><itera:message key="iteraQl.resultType" />
														<c:out value="${iteraQlBean.type1Name}" />
													</th>
												</tr>
												<tr>
													<th colspan="3"><itera:message key="iteraQl.resultCount" />
														<c:out value="${iteraQlBean.type1Size}" />
													</th>
												</tr>
												<tr>
		...
		<input id="doInterchange" type="button" class="link btn" value="<itera:message key="iteraql.interchange.trigger" />" onclick="createHiddenField('selectedBuildingBlock','${memBean.selectedBuildingBlock}');submitForm('interchange.do');" />
