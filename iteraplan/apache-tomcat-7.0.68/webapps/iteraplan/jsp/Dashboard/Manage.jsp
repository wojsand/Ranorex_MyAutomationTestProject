...
<script type="text/javascript">
    var djConfig = {parseOnLoad: true, locale: '<itera:message key="calendar.lang"/>', baseUrl: '<c:out value="${pageContext.request.contextPath}/resources/dojo/" />'};
</script>
...
<c:choose>
					...
					<div class="row-fluid">
						<div class="row-fluid">
							<div class="span12 pager">
								<h5><itera:message key="manageDashboard.numberOfBB" /></h5>
							</div>
						</div>
						<div class="row-fluid">
							<div class="span12 pager">
						...
						<div class="span5">
							<form:select path="isrStatusMap" onchange="createPie('isrStatusMap','bbMap')"  multiple="false">
								<form:option value="attributes" selected="true">
									<itera:message key="reports.selectAttribute"/>
								</form:option>
							</form:select>
						</div>
					</div>
						...
						<div class="row-fluid" style="margin-top: 18px;">
							...
							<!-- <div id="topUsedISR" class="container-fluid"> -->
								<div id="TopIsrModule" class="row-fluid module">
									<div class="module-heading">
										<itera:message key="manageDashboard.topUsedISR" />
									</div>
									<div class="row-fluid">
						...
						<div class="row-fluid">
							...
							<div id="TopUsedTCRModule" class="row-fluid module">
								<div class="module-heading" onclick="$('#TopUsedTCRModule').toggle();$('#TopUsedTCRByADModule').toggle();">
									<i id="collapseIcon" class="icon-resize-full"></i>
                               		&nbsp;<itera:message key="manageDashboard.topUsedTCR" />
								</div>
								<div class="row-fluid">
									<div class="module-body-table">
										<div class="row-fluid">
							...
							<div id="TopUsedTCRByADModule" class="row-fluid module" style="display: none;">
								<div class="module-heading" onclick="$('#TopUsedTCRModule').toggle();$('#TopUsedTCRByADModule').toggle();">
									...
									&nbsp;<itera:message key="manageDashboard.topUsedTCRByAD" />
								</div>
								<div class="row-fluid">
									<div class="module-body-table">
										<div class="row-fluid">
													...
													<c:forEach items="${dialogMemory.topUsedTcrByAdMap}" var="tcrByAdEntry" varStatus="headerStatus">
														<c:choose>
															...
															<c:when test="${headerStatus.index == 0}">
																<tr>
																	<th colspan="2">
																		<itera:message key="manageDashboard.noADAssigned" />
																	</th>
																</tr>
															</c:when>
															<c:otherwise>
							...
							if('<c:out value="${attr.key}"/>' == 'global.type_of_status'){
								<c:forEach items="${attr.value}" var="attrvalue">
									for(var i = 0; i < data.length; i++) {
										if('<itera:message key="${attrvalue.key}"/>' == data[i].text) {
		...
		<c:forEach items="${dialogMemory.bbMap}" var="entry">
			...
			if (bbtype == opt) {
				...
				if ('<c:out value="${fn:length(bbname)}"/>' != '0') {
					<c:forEach items="${bbname}" var="attr">
						if('<c:out value="${attr.key}"/>' == 'global.type_of_status') {
							newOption = new Option('<itera:message key="${attr.key}"/>');
	...
	<c:forEach items="${dialogMemory.bbMap}" var="entry">
		...
		<c:forEach items="${bbname}" var="attr">
			...
			if('<c:out value="${attr.key}"/>'=="global.type_of_status"){
				attrtempval="<itera:message key='${attr.key}'/>";
			}
			else if('<c:out value="${attr.key}"/>'=="seal"){
				attrtempval="<itera:message key='${attr.key}'/>";
			}
			...
			if(attrvalue==attrtempval){
				...
				if('<c:out value="${attr.key}"/>'=="global.type_of_status"){
					<c:forEach items="${attr.value}" var="attrvalue">
						data.push({y:<c:out value="${fn:length(attrvalue.value)}"/>, text:'<itera:message key="${attrvalue.key}"/>', tooltip:<c:out value="${fn:length(attrvalue.value)}" />});
					</c:forEach>
				}
				else if('<c:out value="${attr.key}"/>'=="seal"){
					<c:forEach items="${attr.value}" var="attrvalue">
				...
				if(data.length>0) {
					...
					document.getElementById('pieChart').appendChild(document.createTextNode('<itera:message key="global.dashboard.attributemessage" />'));
				}
    ...
    <c:forEach items="${dialogMemory.bbMap}" var="entry">  
    	<c:if test="${(functionalPermissionBD == true) && (entry.key == 'businessDomain.')}">
    	 		chartData.push({ x: '<itera:message key="${entry.key}singular" />', y: <c:out value="${entry.value}" />, tooltip: '<c:out value="${entry.value}" />&nbsp;<itera:message key="${entry.key}plural" />' });
    	 		nrBB += 1;
    	</c:if>
									...
									{value: 0, text: '' }
                        	         ...
                        	         <c:forEach items="${dialogMemory.bbMap}" var="entry" varStatus="status">
                        	         		<c:if test="${(functionalPermissionBD == true) && (entry.key == 'businessDomain.')}">
                        	         			...
                        	         			, {value:  <c:out value="${axisElementCounter}" />, text: '<itera:message key="${entry.key}abbr" />'}
                        	         		</c:if>
                                	        <c:if test="${(functionalPermissionBP == true) && (entry.key == 'businessProcess.')}">
                                	       		 <c:set var="axisElementCounter" value="${axisElementCounter + 1}" />
                                	        	, {value:  <c:out value="${axisElementCounter}" />, text: '<itera:message key="${entry.key}abbr" />'} 
