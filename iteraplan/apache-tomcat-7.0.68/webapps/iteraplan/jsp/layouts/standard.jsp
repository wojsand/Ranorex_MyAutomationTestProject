...
<tilesx:useAttribute name="title_style" ignore="true" />
<tilesx:useAttribute name="header" />
<tilesx:useAttribute name="navigation" />
<tilesx:useAttribute name="breadcrumb" />
<tilesx:useAttribute name="menu" />
<tilesx:useAttribute name="contents" ignore="true" />
<tilesx:useAttribute name="footer" />

<tilesx:useAttribute name="form_model" ignore="true" />
<tilesx:useAttribute name="form_action" ignore="true" />
<tilesx:useAttribute name="form_id" ignore="true" />
<tilesx:useAttribute name="form_method" ignore="true" />
<tilesx:useAttribute name="form_enctype" ignore="true" />

<tilesx:useAttribute name="hideContextActionsMenu" ignore="true" />
...
<title><itera:message key="${title}" /> <c:choose>
		<c:when
			test="${title_style == 'global.overview' || title_style == 'global.new'}">
					: <itera:message key="${title_style}" />
		</c:when>
		<c:when
	...
	</c:choose> - <itera:message key="global.applicationname" /></title>
...
<jwr:style media="all" src="/bundles/custom.css" />
...
<body>
	...
	<itera:message key="DATE_FORMAT" var="dateformatForJava" />
	...
	<script type="text/javascript">
		...
		function colorpickerInit() {
    		 $('.colorpicker-plainhex').colorpicker({format:'plainhex'})   // input color fields WITHOUT leading #
    		 .add($('.colorpicker-hashhex').colorpicker({format:'hex'}))   // input color fields with leading #
    		 .on('changeColor', function(ev) {
    	    	  var textColor = ev.color.toRGB();
    	    	  var perceptiveLuminance = 1 - ( 0.299 * textColor.r + 0.587 * textColor.g + 0.114 * textColor.b)/255;
    	    	  var colorField = $(this);
    	    	  colorField.css("color", perceptiveLuminance > 0.5 ? '#ffffff' : '#000000');
    	    	  colorField.css("background-color", ev.color.toHex());
    	     }).on('create', function(ev) {
    	    	 $(ev.target).colorpicker('setValue', ev.target.value);
    	     });
		}
	...
	<form:form id="${form_id}" method="${form_method}"
				...
				<c:if test="${functionalPermissionSearch}">
					<div id="searchHeader" class="pull-right" style="padding-top: 6px">
						...
						<i class="icon-search" style="margin-top: 3px;"></i> <input
							...
							title="<itera:message key="menu.quicksearch.inputlabel" />"
							type="text"
						...
						<input type="submit" style="margin-bottom: 5px;"
							...
							value="<itera:message key="button.sendSearchquery" />"
							onclick="submitForm('<c:url value="/search/globalsearch.do" />', 'GET');" />
						
						<c:if test="${icLinksEnabled == 'true'}">
							<a style ="margin-left: 108px;" class="nclink btn" 
							href="<c:url value="/client/" />"><itera:message 
							key="menu.header.toNCHome" /></a>
						</c:if>
					</div>
				</c:if>
		...
		<div class="page">
			...
			<div class="container-fluid" style="padding: 0 10px;">
				<div class="row-fluid">
                    <!--
                        Context actions menu can be hidden:
                          a) explicitly: by setting the attribute 'contextActionsMenuDisplay' to true in the view definition.
                          b) implicitly: when the active dialog in the GuiContext is null or ""
                    -->
					<c:set var="contextActionsMenuDisplay" value="" />
					<c:set var="contentStyle" value="" />
					<c:if test="${(hideContextActionsMenu == true) or (empty guiContext.activeDialogName)}">
						<c:set var="startPageStyle"
							value="style=\"margin: auto; width: 100%;\"" />
						<c:set var="contextActionsMenuDisplay" value="hidden" />
					</c:if>
					<c:if test="${contextActionsMenuDisplay != 'hidden'}">
						...
						<spring:hasBindErrors name="${form_model}">
							...
							<div class="alert alert-error">
								<a class="close" data-dismiss="alert">×</a> <span
									class="errorHeader"><itera:message key="errors.header" /></span><br />
