...
<c:set var="functionalPermission" scope="request">
	<c:set var="infrastructureElement" value="infrastructureElement.singular" />
	<itera:write name="userContext"
		property="perms.userHasBbTypeFunctionalPermission(${infrastructureElement})" escapeXml="false" />
</c:set>
...
<div class="${componentModeClass}">
	...
	<c:when test="${functionalPermission == true}">
		...
		<tiles:insertTemplate template="/jsp/common/TransactionBar.jsp">
			...
			<tiles:putAttribute name="service_class" value="InfrastructureElementService" />
		...
		<tiles:insertTemplate template="/jsp/common/ElementNameComponentView.jsp">
			<tiles:putAttribute name="icon_suffix" value="infrastructureelement" />
		...
		<itera:tabgroup id="ConfigTabs" activeTab="${activeTab}">
		  	<itera:tab id="TabHierarchy" 	textKey="global.hierarchy" 		page="/jsp/InfrastructureElement/tabPages/TabHierarchy.jsp" showTab="true" />
		  	<itera:tab id="TabAssignments" 	textKey="button.assignments" 	page="/jsp/InfrastructureElement/tabPages/TabAssignments.jsp" showTab="${showTab}" />
		  	...
		  	<c:if test="${componentMode == 'READ'}">
		  		<itera:tab id="FastExport"  textKey="global.fastExport" 	page="/jsp/InfrastructureElement/tabPages/TabFastExportInfrastructureElement.jsp" />
		  	</c:if>
			<c:if test="${componentMode == 'READ' && isHistoryEnabled}">
				<itera:tab id="TabLocalHistory" textKey="global.history"    page="/jsp/InfrastructureElement/tabPages/TabHistory.jsp"/>
			</c:if> 
		</itera:tabgroup>
