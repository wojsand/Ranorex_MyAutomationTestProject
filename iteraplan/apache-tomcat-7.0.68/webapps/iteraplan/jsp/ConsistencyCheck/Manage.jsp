...
<c:choose>
	<c:when test="${functionalPermission == true}">
		...
		<c:forEach items="${dialogMemory.domains}" var="domain">
			...
			<div class="row-fluid module">
				<div class="module-heading">
					<itera:message key="manageConsistencyReport.title" /> - <itera:message key="${domain.value}" />
					&nbsp;
					<input type="button" class="btn" value="<itera:message key="button.run" />" onclick="createHiddenField('selectedConsistencyCheck', '<c:out value="${domain}"/>');return submitForm('requestFullDomainReport.do');" />
				</div>
				<div class="row-fluid">
							...
							<table class="searchResultView table table-striped table-condensed tableInModule">
								...
								<thead>
									<tr>
										...
										<th><itera:message key="check.name" /></th>
										<th>&nbsp;</th>
									</tr>
								</thead>
								<tbody>
									...
									<c:forEach items="${dialogMemory.configurations[domain]}" var="configuration" varStatus="loopStatus">
										...
										<tr>
											...
											<td class="col-ico">
												<input type="button" value="<itera:message key="button.run" />" class="btn" onclick="createHiddenField('selectedConsistencyCheck', '<c:out value="${configuration.name}"/>');createHiddenField('selectedConsistencyCheckNumber', '<c:out value="${index}"/>');return submitForm('requestSingleReport.do');" />
											</td>
										</tr>
									</c:forEach>
								</tbody>
		...
		<input type="button" value="<itera:message key="button.generateReport" />" class="btn" onclick="createHiddenField('selectedConsistencyCheck', '<c:out value="${domain}"/>');return submitForm('requestFullReport.do');" />
	</c:when>
	<c:otherwise>
