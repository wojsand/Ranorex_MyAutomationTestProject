...
<c:set var="functionalPermission" scope="request">
	<c:set var="businessProcess" value="businessProcess.singular" />
	<itera:write name="userContext"
		property="perms.userHasBbTypeFunctionalPermission(${businessProcess})" escapeXml="false" />
</c:set>
	...
	<c:choose>
		<c:when test="${functionalPermission == true}">
			...
			<tiles:insertTemplate template="/jsp/common/TransactionBar.jsp" >
				...
				<tiles:putAttribute name="service_class" value="BusinessProcessService" />
			...
			<tiles:insertTemplate template="/jsp/common/ElementNameComponentView.jsp">
				<tiles:putAttribute name="icon_suffix" value="businessprocess" />
			...
			<itera:tabgroup id="ConfigTabs" activeTab="${activeTab}">
				...
				<itera:tab id="TabAssignments" 	textKey="button.assignments" 	page="/jsp/BusinessProcess/tabPages/TabAssignments.jsp" showTab="${showTab}"/>
				...
				<c:if test="${componentMode == 'READ'}">
					<itera:tab id="FastExport"  textKey="global.fastExport"     page="/jsp/BusinessProcess/tabPages/TabFastExportBusinessProcess.jsp" />
				</c:if>	
				<c:if test="${componentMode == 'READ' && isHistoryEnabled}">
					<itera:tab id="TabLocalHistory" textKey="global.history"    page="/jsp/BusinessProcess/tabPages/TabHistory.jsp"/>
				</c:if> 
			</itera:tabgroup>
