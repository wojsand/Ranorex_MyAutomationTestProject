...
<c:set var="functionalPermission" scope="request">
	<c:set var="informationSystemDomain" value="informationSystemDomain.singular" />
	<itera:write name="userContext"
		property="perms.userHasBbTypeFunctionalPermission(${informationSystemDomain})" escapeXml="false" />
</c:set>
	...
	<c:choose>
		<c:when test="${functionalPermission == true}">
			...
			<tiles:insertTemplate template="/jsp/common/TransactionBar.jsp">
				...
				<tiles:putAttribute name="service_class" value="InformationSystemDomainService" />
			...
			<tiles:insertTemplate template="/jsp/common/ElementNameComponentView.jsp">
			    <tiles:putAttribute name="icon_suffix" value="informationsystemdomain" />
			...
			<itera:tabgroup id="ConfigTabs" activeTab="${activeTab}">
			  	...
			  	<itera:tab id="TabAssignments" 	textKey="button.assignments" 	page="/jsp/InformationSystemDomain/tabPages/TabAssignments.jsp" showTab="${showTab}" />
			  	...
			  	<c:if test="${componentMode == 'READ'}">
			  		<itera:tab id="FastExport"  textKey="global.fastExport" 	page="/jsp/InformationSystemDomain/tabPages/TabFastExportInformationSystemDomain.jsp" />
				</c:if>
				<c:if test="${componentMode == 'READ' && isHistoryEnabled}">
					<itera:tab id="TabLocalHistory" textKey="global.history"    page="/jsp/InformationSystemDomain/tabPages/TabHistory.jsp"/>
				</c:if> 
			</itera:tabgroup>
