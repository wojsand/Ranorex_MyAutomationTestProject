...
<div id="OverviewHeadingContainer" class="row-fluid module">
	<div class="module-heading">
	    <itera:message key="${header_key}" /> - <span style="font-weight:normal"><itera:message key="global.overview.subheader" /></span>
	</div>
</div>
...
<c:choose>
	<c:when test="${functionalPermission == true}">
		<c:forEach items="${dialogMemory.overviewLists.lists}" var="overviewList" varStatus="listsCount">
			...
			<c:if test="${((listsCount.index) % numberOfColumns == numberOfColumns - 1) or listsCount.last}">
			   </div>
			</c:if>
		</c:forEach>
    
        <div>
		   <span class="helpText">1. <itera:message key="global.overview.footnote.informationSystem" /></span> <br>
		   <span class="helpText">2. <itera:message key="global.overview.footnote.technicalComponent" /></span>
       </div>
	</c:when>
	<c:otherwise>
