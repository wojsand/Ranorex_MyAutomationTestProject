...
<div class="row">
	<div id="addDateIntervalDiv" class="btn-toolbar pull-right" style="padding: right;">
		<a href="#" onclick="${createNewJavascript}" class="btn btn-primary"><i class="icon-plus icon-white"></i> <itera:message key="button.create" /></a>
	</div>
</div>
...
<div id="ResultTableModule" class="row-fluid module">
	<div class="module-heading-nopadding">
		<div style="padding: 8px 0px; float: left;">
			<itera:message key="global.dateintervals" />
		</div>
	</div>
	<div class="row-fluid">
		<div class="module-body-table">
			...
			<table class="table table-striped table-condensed tableInModule">
				...
				<thead>
					<tr>                  
						<th nowrap="nowrap"><itera:message key="global.name" /></th>
						<th nowrap="nowrap"><itera:message key="global.startfordateinterval" /></th>
						<th nowrap="nowrap"><itera:message key="global.endfordateinterval" /></th>
						<th nowrap="nowrap"><itera:message key="reports.color" /></th>
						<th nowrap="nowrap"><itera:message key="global.edit" /></th>
					</tr>
				</thead>
				<tbody>
					...
					<c:forEach var="row" items="${dialogMemory.intervals}">
						<tr>
							...
							<td>
								<a href="javascript:changeLocation(getRestURIRelativeToMVC()+'<c:out value="${row.id}" />?_eventId=edit');" title="<itera:message key='button.edit'/>">
								<i class="icon-pencil"></i>
								</a>
								<a href="javascript:confirmDelete(function(){changeLocation(getRestURIRelativeToMVC()+'<c:out value="${row.id}" />?_eventId=delete')});" title="<itera:message key='button.delete'/>">
