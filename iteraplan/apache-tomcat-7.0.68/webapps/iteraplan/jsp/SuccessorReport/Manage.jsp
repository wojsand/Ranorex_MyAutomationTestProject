		...
		<ul class="nav nav-tabs">
			<li class="${informationSystemSuccessorsActive}">
				<a href="#tab_informationSystemSuccessors" data-toggle="tab" onclick="GuiService.setSelectedTab('informationSystemSuccessors');">
					<itera:message key="reports.successors.informationsystems.title" />
				</a>
			</li>
			...
			<li class="${technicalComponentSuccessorsActive}">
				<a href="#tab_technicalComponentSuccessors" data-toggle="tab" onclick="GuiService.setSelectedTab('technicalComponentSuccessors');">
					<itera:message key="reports.successors.technicalcomponents.title" />
				</a>
			</li>
		</ul>
	</c:if>
		...
		<c:if test="${functionalPermissionISR == true}">
			<div class="tab-pane ${informationSystemSuccessorsActive}" id="tab_informationSystemSuccessors">
				...
				<div class="row-fluid">
				    ...
				    <itera:message key="reports.successors.showSuccessors" />&nbsp;&nbsp;&nbsp;&nbsp;
				    <form:radiobutton path="isrSuccessorDTO.showSuccessor" value="false" cssStyle="vertical-align:middle;margin: 0px 3px 0px 0px;"/>
				    <itera:message key="reports.successors.showPredecessors" />
				</div>
				<div class="row-fluid">
				    <itera:message key="reports.resultFormat" />&nbsp;
					<form:select path="isrSuccessorDTO.selectedResultFormat">
			        	<c:forEach var="availableResultFormat" items="${dialogMemory.availableResultFormats}">
			          		<form:option value="${availableResultFormat.presentationKey}">
			            		<itera:message key="${availableResultFormat.presentationKey}" />
			          		</form:option>
			        	</c:forEach>
					</form:select>
					&nbsp;
					<a href="#" rel="tooltip" data-original-title="Hotkey: <img/> <itera:message key='button.sendQuery.tooltip'/>">
						<input type="submit" id="<itera:message key='button.sendQuery.id'/>" class="link btn btn-primary" value="<itera:message key="button.sendQuery" />"	
			...
			<div class="tab-pane ${technicalComponentSuccessorsActive}" id="tab_technicalComponentSuccessors">
				...
				<div class="row-fluid">
				    <itera:message key="reports.resultFormat" />&nbsp;
		          	...
		          	&nbsp;
		          	<a href="#" rel="tooltip" data-original-title="Hotkey: <img/> <itera:message key='button.sendQuery2.tooltip'/>">
						<input type="submit" id="<itera:message key='button.sendQuery2.id'/>" class="link btn btn-primary" value="<itera:message key="button.sendQuery" />" 
