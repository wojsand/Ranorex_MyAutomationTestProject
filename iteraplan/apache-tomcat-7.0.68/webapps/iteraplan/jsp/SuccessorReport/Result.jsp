...
<c:choose>
  ...
  <c:when test="${dialogMemory.isrSuccessorDTO != null && dialogMemory.isrSuccessorDTO.noQueryExecuted
    ...
    <p><itera:message key="reports.results.resultListIsEmtpy" /></p>
  </c:when>
  <%-- no releases available for querying --%>
  <c:when test="${(dialogMemory.selectedIsrId == null) && (dialogMemory.selectedTcrId == null)}">
    <p><itera:message key="message.report.successor.noReleaseAvaliable" /></p>
  </c:when>  
</c:choose>
...
<c:if test="${queryResultDto != null && queryResultDto.succession[0] != null && 
	...
	<div id="SuccessorReportResultsModule" class="row-fluid module">
		<div class="module-heading">
			<itera:message key="reports.results" />
		</div>
		<div class="row-fluid">
			<div class="module-body-table">
				<div class="row-fluid">
					<table class="table table-striped table-condensed tableInModule">
						<thead>
							<tr>
								<th><itera:message key="global.nameversion" /></th>
								<th><itera:message key="global.from" /></th>
								<th><itera:message key="global.to" /></th>
								<th><itera:message key="global.type_of_status" /></th>
								<c:if test="${showLinkedProjects}">
            						<th><itera:message key="project.singular" /></th>
          						</c:if>
							</tr>
						</thead>
						<tbody>
						    ...
						    <c:forEach var="container" items="${queryResultDto.succession}">
						        ...
						        <tr class="<c:out value="${linkStyle}" />" onclick="${linkScript}" >
						        	<td style="white-space: nowrap;">
						          	...
						          	<td class="statusintable"><itera:message key="${release.typeOfStatusAsString}" /></td>
						          	<c:if test="${showLinkedProjects}">
						            	<td>
