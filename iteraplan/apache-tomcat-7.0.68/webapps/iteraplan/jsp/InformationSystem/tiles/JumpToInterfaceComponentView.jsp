...
<tilesx:useAttribute name="showNewConnectionLink" ignore="true" />
<tilesx:useAttribute name="showExistingConnectionLinks" ignore="true" />
...
<div id="JumpToInterfaceComponentViewModul" class="row-fluid module">
	<div class="module-heading">
		<itera:message key="informationSystemRelease.to.interfaces" />
	</div>
	<div class="row-fluid">
					...
					<c:if test="${componentMode == 'CREATE'}">
						<tr>
							<td>
								...
								<span><itera:message key="informationSystemRelease.new.interface" />
								</span></div>
							</td>
						</tr>		
					</c:if>
						...
						<c:choose>
							...
							<c:otherwise>
								<tr>
									...
									<th>
										<itera:message key="global.direction" />
									</th>
									<th>
										<itera:message key="informationSystemRelease.interfaceTo" />
									</th>
									<th>
										<itera:message key="interface.transport.businessObjects" />
									</th>
									<th>
										<itera:message key="technicalRealisation" />
									</th>
									<th>&nbsp;</th>
								</tr>
							</c:otherwise>
						...
						<c:if test="${userContext.perms.userAllowedToCreateConnections && showNewConnectionLink}">
							<tr id="<c:out value="${html_id}_new" />">
								<td colspan="5" align="left" class="borderTop">
									<a href="#" rel="tooltip" data-original-title="Hotkey: <img/> <itera:message key="jumpToInterface.tooltip"/>">
										<input type="button" class="link btn" value="<itera:message key="button.newConn" />" 
