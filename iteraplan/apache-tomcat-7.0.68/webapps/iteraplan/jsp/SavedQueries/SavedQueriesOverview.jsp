...
<script type="text/javascript">
  ...
  function deleteSavedQuery(queryName, queryId) {
    showConfirmDialog("<itera:message key='global.confirmDelete'/>",
        "<itera:message key='graphicalReport.executeRemoveQuery'><itera:param>"
            + queryName + "</itera:param></itera:message>", function() {
  ...
  <c:when test="${functionalPermission}">
    ...
    <div id="ShowSafedQueryContainer" class="row-fluid module">
      <div id="savedQueryHeading" class="module-heading" style="height:22px">
        ...
        <itera:message var="savedQueriesValue" key="graphicalReport.savedQueries" />
        <itera:message var="found" key="search.found.header" />
            ...
            <div id="savedQueriesScrollBox" class="row-fluid">
              <table id="ShowSavedQueryTable" class="table table-striped table-condensed tableInModule">
                  ...
                  <tr>
                    ...
                    <th class="col-desc"><itera:message key="global.reportType"/></th>
                    <th class="col-desc"><itera:message key="global.bbtype"/></th>
                    <th class="col-desc"><itera:message key="global.link"/></th>
                    <c:if test="${permissionSaveAnyReports}">
                      <th class="col-desc"><itera:message key="button.delete"/></th>
                    </c:if>
                  </tr>
                </thead>
                  <tbody>
                    <c:forEach items="${savedQueries}" var="savedQuery">
                      ...
                      <tr style="${rowStyle}">
                        ...
                        <td onclick="${loadQueryOnclickValue}">
                          <a href="#" rel="tooltip" data-original-title="<itera:message key='graphicalReport.loadSavedQuery.tooltip'/>">
                            <c:out value="${query_name}" escapeXml="true" />
                          </a>
                        </td>
                        ...
                        <td onclick="${loadQueryOnclickValue}">
                          <a href="#" rel="tooltip" data-original-title="<itera:message key='graphicalReport.loadSavedQuery.tooltip'/>">
                            <itera:message key="${savedQuery.type.titleProperty}" />
                          </a>
                        </td>
                        ...
                        <%-- "Link" action --%>
                        <td>
                          <a href="#" onclick="showTipLinkDialog('<itera:message key="global.bookmark" />:', '<itera:message key="global.bookmark" />', '${fastExportURLforBookmark}');">
