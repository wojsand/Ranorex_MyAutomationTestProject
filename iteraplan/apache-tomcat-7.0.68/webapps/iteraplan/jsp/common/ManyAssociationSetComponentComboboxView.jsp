...
--%>
<tilesx:useAttribute name="connected_elements_field_styles" ignore="true" />
...
--%>
<tilesx:useAttribute name="combobox_label_values" ignore="true" />

<%-- 
  The field in which the selected value of the combobox is stored. 
  See combobox_label_values attribute. 
--%>
<tilesx:useAttribute name="combobox_field" ignore="true" />

<%-- String appended to the header of the table when in READ mode. --%>
<tilesx:useAttribute name="header_postfix_read" ignore="true" />

<%-- String appended to the header of the table when in EDIT mode. --%>
<tilesx:useAttribute name="header_postfix_edit" ignore="true" />
...
--%>
<tilesx:useAttribute name="info_message_key" ignore="true" />
...
--%>
<tilesx:useAttribute name="info_message_key_empty" ignore="true" />
...
<tilesx:useAttribute name="addColumColspan" ignore="true" />

<tilesx:useAttribute name="dynamically_loaded" ignore="true" />
...
<div id="${html_id}_modul" class="row-fluid module">
		<div class="module-heading">
			<itera:message key="${table_header_key}" /> <c:out value="${postfix}" escapeXml="false" />
		</div>
						...
						<c:if test="${connected_elements_size > 0 && empty minimal}">
							<tr>
								...
								<c:forEach items="${column_header_keys}" var="column_header_key">
									<th><itera:message key="${column_header_key}" /></th>
								</c:forEach>
								<c:if test="${not empty isAttributable && isAttributable}">
									<c:forEach items="${attribute_types}" var="attribute_type" varStatus="atCount">
										...
										<c:if test="${componentMode != 'READ'}">
											...
											<c:if test="${!writePermissionATG}">
												<div class="helpText">
													<itera:message key="messages.noAtGroupWritePermission.association">
														<itera:param value="${attribute_type.name}" />
													</itera:message>
												</div>
											</c:if>
										</c:if>
								...
								<c:if test="${viewMode != 'READ'}">
									<td>
										<a id="<c:out value="${html_id}_${elCount.index}_remove" />"
											...
											onclick="confirmDeleteAttributeValueAssignment('<c:out value="${remove_id_field}"/>','<c:out value="${connected_element.id}"/>', function() {flowAction('update');}, '<c:out value="${askBeforeDelete}"/>','<itera:message key="global.confirmDelete"/>','<itera:message key="${confirmDeleteAttributeValueAssignmentMessageKey}"/>');" >
									...
									<c:choose>
										...
										<c:when test="${column_header_keys[count] == 'manageAttributes.defaultcolor'}">
											...
											<c:set var="output">
												<c:choose>
													<c:when test="${viewMode != 'READ'}">
														<%-- select box to choose the color --%>
														...
														<tiles:insertTemplate template="/jsp/common/ColorSelectTile.jsp">
															<tiles:putAttribute name="colorPath" value="${path_to_componentModel}.valueToColorMap['${connected_element.identityString}']"/>
						...
						<c:if test="${viewMode != 'READ'}">
							<tr>
								<td><input type="hidden" name="<c:out value="${remove_id_field}"/>" value="" />
									<a id="<c:out value="${html_id}_add"/>" class="link <c:out value="${html_id}_add" />"  title="<itera:message key="tooltip.add"/>" onclick="flowAction('update');" >
										<i class="icon-plus"></i>
									</a>
								</td>
								<td class="dontwrap">
									...
									<c:choose>
												...
												<c:forEach items="${available_elements_presentation_obj}" var="available_element">
													...
													<form:option value="${available_element.id}">
														<c:if test="${available_key != ''}">
															<itera:message key="${available_key}" />
														</c:if>
													</form:option>
												</c:forEach>
											</form:select>
										...
										<c:when test="${available_elements_presentation_group_keys != null}">
											<form:select path="${add_id_field}" id="${html_id}_select" required="false" cssClass="name combobox">
												...
												<c:forEach items="${available_elements_presentation_obj}" var="available_group" varStatus="indexStatus">
													<option value="">- <itera:message	key="${available_elements_presentation_group_keys[indexStatus.index]}" /> -</option>
										...
										<c:otherwise>
											<c:if test="${dynamically_loaded == true}">
												...
												<itera:define id="ownBbType" name="memBean" property="typeOfBuildingBlock" />
											</c:if>
											<form:select path="${add_id_field}" id="${html_id}_select" cssClass="name combobox" dynLoad="${dynamically_loaded}" bbtype="${fn:toLowerCase(bbtype)}" ownbbtype="${fn:toLowerCase(ownBbType)}" >
												<itera:define id="available_elements_presentation_obj" name="memBean" property="${available_elements_presentation}" />
									...
									<c:when test="${path_to_componentModel == 'componentModel.responsibilityAttributeValuesModel'}">
										...
										<td class="color">
											...
											<tiles:insertTemplate template="/jsp/common/ColorSelectTile.jsp">
												...
												<tiles:putAttribute name="availableColors" value="${availableColors_create}"/>
											</tiles:insertTemplate>
										</td>
										<td></td>
									</c:when>
