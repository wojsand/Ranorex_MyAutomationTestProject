...
<tilesx:useAttribute name="subscribed_element" ignore="true"/>
<tilesx:useAttribute name="element_id" ignore="true"/>

<tilesx:useAttribute name="userHasFuncPermission" ignore="true" />
		...
		function confirmDeleteAttribute(list, callback){
		    if(list.length > 0) {
			   ...
			   showConfirmDialog('<itera:message key="global.confirmDelete"/>',
					   '<itera:message key="attribute.delete1"/> <br />- ' + buildingBlocks + '<br /> <br />' + '<itera:message key="attribute.delete2"/> ',
					   callback);
		    } else {
		    	callback();
		    }
	...
	<c:otherwise>
		<c:set var="updatePermissionType">
			<itera:write name="userContext"
				property="perms.userHasBbTypeUpdatePermission(${memBean.componentModel.typePresentationKey})" escapeXml="false" />
		</c:set>
		<c:set var="createPermissionType">
			<itera:write name="userContext"
				property="perms.userHasBbTypeCreatePermission(${memBean.componentModel.typePresentationKey})" escapeXml="false" />
		</c:set>
		<c:set var="deletePermissionType">
			<itera:write name="userContext"
				property="perms.userHasBbTypeDeletePermission(${memBean.componentModel.typePresentationKey})" escapeXml="false" />
		</c:set>
	...
	<div class="btn-group">
		...
		<c:if test="${componentMode == 'READ'}">
				...
				<c:choose>
					<c:when test="${sealState == 'seal.valid'}">
						<a data-toggle="modal" class="btn" href="#SealContainer" >
							...
							<itera:message key="seal"/>&nbsp;<itera:message key="seal.valid"/>
						</a>
					</c:when>
					<c:when test="${sealState == 'seal.outdated'}">
						<a data-toggle="modal" class="btn" href="#SealContainer" >
							<i class="icon-ban-circle"></i>
							<itera:message key="seal"/>&nbsp;<itera:message key="seal.outdated"/>
						</a>
					</c:when>
					<c:when test="${sealState == 'seal.invalid'}">
						<a data-toggle="modal" class="btn" href="#SealContainer" >
							<i class="icon-ban-circle"></i>
							<itera:message key="seal"/>&nbsp;<itera:message key="seal.invalid"/>
						</a>
					</c:when>
					<c:when test="${sealState == 'seal.notavailable'}">
						<a href="#" class="btn" >
							<i class="icon-ban-circle"></i>
							<itera:message key="seal"/>&nbsp;<itera:message key="seal.notavailable"/>
						</a>
					</c:when>
				</c:choose>		
			</c:if>
			...
			<a class="btn dropdown-toggle" data-toggle="dropdown" href="#">
				<i class="icon-cog"></i>
				<itera:message key="menu.header.more" />&nbsp;<span class="caret"></span>
			</a>
			<ul class="dropdown-menu dropdown-right">
				...
				<li>
					<a id="transactionRefresh" href="#"
						...
						<itera:message key="button.refresh" />
					</a>
				</li>
				  	...
				  	<c:if test="${subscribePermission}">
						<li>
							<a href="javascript:doSubscribe();">
								...
								<span class="subscribe_button"><itera:message key="global.subscribe"/></span>
							</a>
						</li>
					</c:if>
					<c:if test="${showSubscribersPermission}">
						<li>
							<a id="subscribed_users_count" href="javascript:showSubscribedUsers();">
								<i class="icon-user"></i>
								<itera:message key="global.subscribed.users.show" />&nbsp;<span class="subscribercount">(0)</span>  <%-- will be updated by JavaScript after page has loaded completely, so a static value is okay --%>
							</a>
						</li>
					</c:if>
				</c:if>
					...
					<c:if test="${showNewReleaseButton && createPermission}">
						<li>
							<a id="transactionNewRelease" href="#"
								...
								<itera:message key="button.newRelease" />
							</a>
						</li>
					</c:if>
		...
		<c:if test="${componentMode == 'EDIT' || componentMode == 'CREATE'}">
			...
			<%-- SAVE --%>
<!-- There always happens a page reload after save, so the submit lock time can be high to ensure no double-clicking for longer-duration save processes -->
			<a id="transactionSave" class="btn btn-primary" href="#"
				onclick="flowAction('save', 20000);" >
