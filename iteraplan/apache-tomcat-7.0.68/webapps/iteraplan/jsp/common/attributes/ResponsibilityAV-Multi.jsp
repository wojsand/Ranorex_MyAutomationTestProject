	...
	<div class="control-group">
		...
		<c:choose>
			<c:when test="${atPart.componentMode != 'READ' && writePermissionATG && overviewMode == false}">
				<tiles:insertTemplate template="/jsp/common/attributes/tiles/AddRemoveResponsibilityAVs-Multi.jsp" flush="true">
					<tiles:putAttribute name="atPartPath" value="${atPartPath}" />
					<tiles:putAttribute name="extended_html_id" value="${extended_html_id}" />
				</tiles:insertTemplate>
			</c:when>
			<c:otherwise>
				<tiles:insertTemplate template="/jsp/common/attributes/tiles/ConnectedResponsibilityAVs-Multi.jsp" flush="true">
					<tiles:putAttribute name="atPartPath" value="${atPartPath}" />
					<tiles:putAttribute name="extended_html_id" value="${extended_html_id}" />
				</tiles:insertTemplate>
			</c:otherwise>
