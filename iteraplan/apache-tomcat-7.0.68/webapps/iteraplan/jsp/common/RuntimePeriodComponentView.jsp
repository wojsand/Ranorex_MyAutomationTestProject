...
<tilesx:useAttribute name="lineCount" ignore="true"/>
<tilesx:useAttribute name="minimal" ignore="true" />
<tilesx:useAttribute name="path_to_componentModelName" ignore="true" />
		...
		<c:when test="${not showDivView}">	
		  	<c:if test="${empty minimal}">
		  		<label class="control-label" for="${html_id}_date">
		  			<itera:message key="${localized_label}" />:
		  		</label>
			</c:if>		
			      	...
			      	<c:otherwise>
			        	...
			        	<c:choose>
							...
							<c:when test="${empty start_date_bean && not empty end_date_bean}">
					        	<c:set var="desc2">
					            	<itera:message key="global.ending"/>&nbsp;
					            </c:set>
					        </c:when>
			          		<c:when test="${not empty start_date_bean && empty end_date_bean}">
			            		<c:set var="desc1">
			              			<itera:message key="global.starting"/>&nbsp;
			            		</c:set>
			          		</c:when>
		...
		<c:otherwise>
			<div class="controls">
				...
				<c:choose>
			        ...
			        <c:when test="${not empty start_date_bean && empty end_date_bean}">
			        	<c:set var="desc1"><itera:message key="global.starting"/>&nbsp;</c:set>
			        </c:when>
			        <c:when test="${empty start_date_bean && not empty end_date_bean}">
			        	<c:set var="desc2"><itera:message key="global.ending"/>&nbsp;</c:set>
			        </c:when>
			        <c:when test="${not empty start_date_bean && not empty end_date_bean}">
			        	<c:set var="desc2"><itera:message key="global.to"/>:</c:set>
			        </c:when>
				</c:choose>
