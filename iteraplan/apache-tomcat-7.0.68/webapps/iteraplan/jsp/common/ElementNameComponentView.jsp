...
<tilesx:useAttribute name="name_field_path" ignore="true"/>
<tilesx:useAttribute name="icon_suffix" ignore="true"/>
...
<tilesx:useAttribute name="nameFieldIsMandatory" ignore="true" />
<tilesx:useAttribute name="validate" ignore="true" />
<tilesx:useAttribute name="prefixKey" ignore="true" />
...
<c:choose>
  <c:when test="${(componentMode == 'EDIT' || componentMode == 'CREATE') && overviewMode }">
    ...
    <span class="headerIcon icon_${icon_suffix }" style="margin-top: 5px;"></span>
   	...
   	<c:if test="${not empty prefixKey}"><itera:message key="${prefixKey}"/>: </c:if>
	<div class="control-group">
		<div class="controls">
			<span rel="tooltip" title="<itera:message key="${name_field_label}" />">
  ...
  <c:otherwise>
    ...
    <h1><c:if test="${not empty prefixKey}"><itera:message key="${prefixKey}"/>: </c:if><itera:write name="memBean" property="${name_field_path}" escapeXml="true" />&nbsp;</h1>
  </c:otherwise>
</c:choose>
