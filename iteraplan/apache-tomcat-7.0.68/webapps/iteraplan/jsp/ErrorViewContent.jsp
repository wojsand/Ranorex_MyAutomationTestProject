...
<p>
	<strong> <itera:message key="errorview.text.technicalError" />
	</strong>
</p>
...
<div class="alert alert-error">
	...
	<%-- if an unexpected exception occurs, rather than displaying an empty box, a general exception message will be shown --%>
		<c:choose>
			...
			<c:otherwise>
				<span><itera:message key="GENERAL_TECHNICAL_ERROR" /></span>
			</c:otherwise>
		</c:choose>
...
<p>
	<itera:message key="errorview.text.helpOne.partOne" />
</p>
<ul>
	<li><itera:message key="errorview.text.helpOne.partTwo" /></li>
	<li><itera:message key="errorview.text.helpOne.partThree" /></li>
	<li><itera:message key="errorview.text.helpOne.partFour" /></li>
</ul>

<p>
	<itera:message key="errorview.text.helpTwo.partOne" />
</p>
...
<c:if test="${not empty _iteraplan_problem_report_enabled}">      
  <c:catch>
    ...
    <p>
      <strong><itera:message key="problemreport.caption" />:</strong>
    </p>
    ...
    <c:if test="${userContext.perms.userIsAdministrator}">
      <p>
        <a href="${storedReports[currentKey].downloadLink}" class="btn btn-primary">
          ...
          <itera:message key="problemreport.download.button.text"></itera:message>
        </a>
      </p>
    </c:if>
...
<c:out value="${reportParts[entry.key]}" default="*empty*" escapeXml="false" />
      ...
      $('#generateMailtoLink').click(function() {
        ...
        $.ajax({
          ...
          type: 'GET'
			...
			clickedElem.text('<itera:message key="problemreport.generate.button.failed"></itera:message>').removeClass('btn-primary').addClass('disabled').unbind('click');
        });
    ...
    <p>
      <button type="button" id="generateMailtoLink" class="btn btn-primary">
        <i class="icon-briefcase"></i> <itera:message key="problemreport.generate.button.text"></itera:message>
      </button>
    </p>
