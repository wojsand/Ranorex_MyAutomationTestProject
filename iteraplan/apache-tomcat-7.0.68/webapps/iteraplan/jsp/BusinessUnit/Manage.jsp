...
<c:set var="functionalPermission" scope="request">
	<c:set var="businessUnit" value="businessUnit.singular" />
	<itera:write name="userContext"
		property="perms.userHasBbTypeFunctionalPermission(${businessUnit})" escapeXml="false" />
</c:set>
	...
	<c:choose>
		<c:when test="${functionalPermission == true}">
			...
			<tiles:insertTemplate template="/jsp/common/TransactionBar.jsp" >
				...
				<tiles:putAttribute name="service_class" value="BusinessUnitService" />
			...
			<tiles:insertTemplate template="/jsp/common/ElementNameComponentView.jsp" flush="true">
				<tiles:putAttribute name="icon_suffix" value="businessunit" />
			...
			<itera:tabgroup id="ConfigTabs" activeTab="${activeTab}">
			  	<itera:tab id="TabHierarchy" 	textKey="global.hierarchy" 		page="/jsp/common/TabHierarchyConsistsOf.jsp" showTab="true"/>
			  	<itera:tab id="TabAssignments" 	textKey="button.assignments" 	page="/jsp/BusinessUnit/tabPages/TabAssignments.jsp" showTab="${showTab}"/>    
			  	...
			  	<c:if test="${componentMode == 'READ'}">
			  		<itera:tab id="FastExport"  textKey="global.fastExport" 	page="/jsp/BusinessUnit/tabPages/TabFastExportBusinessUnit.jsp" />
			  	</c:if>
				<c:if test="${componentMode == 'READ' && isHistoryEnabled}">
					<itera:tab id="TabLocalHistory" textKey="global.history"    page="/jsp/BusinessUnit/tabPages/TabHistory.jsp"/>
				</c:if> 
			</itera:tabgroup>
			...
			<c:if test="${componentMode != 'READ'}">
				<tiles:insertTemplate template="/jsp/common/TransactionBar.jsp">
					<tiles:putAttribute name="showBuildingBlockCopyButton" value="true"/>
					<tiles:putAttribute name="sub_elements_affected_by_delete" value="${memBean.componentModel.subElementCount}"/>
					<tiles:putAttribute name="service_class" value="BusinessUnitService" />
					<tiles:putAttribute name="subscribed_element" value="${memBean.componentModel}" />
					<tiles:putAttribute name="element_id" value="${id}" />
				</tiles:insertTemplate>
