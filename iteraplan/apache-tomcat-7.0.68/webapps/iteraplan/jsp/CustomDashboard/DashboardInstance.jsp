...
<c:choose>
	<c:when test="${functionalPermission}">
		<div class="row-fluid">
			<h1><itera:message key="customDashboard.title" />: <c:out value="${dialogMemory.customDashboardInstance.name}" escapeXml="true" /> </h1>
			
			<itera:message var="bbTPlural" key="${dialogMemory.customDashboardInstance.query.resultBbType.typeOfBuildingBlock.pluralValue}" />
			...
			<div class="row-fluid"><itera:message key="customDashboard.author" />: <a href="${userLink}" ><c:out value="${dialogMemory.customDashboardInstance.author}" /></a></div>
			<div class="row-fluid"><itera:message key="customDashboard.creationTime" />: <fmt:formatDate value="${dialogMemory.customDashboardInstance.creationTime}" pattern="${dateFormat}" /></div>
		</div>
	</c:when>
	<c:otherwise>
