...
<div class="row-fluid module">
	<div class="module-heading"><itera:message key="customDashboard.template.title" />
	</div>
	<div class="module-body-table">
		<table class="table table-striped table-condensed tableInModule">
			<tr>
				<th><itera:message key="global.name" /></th>
				<th><itera:message key="global.bbtype" /></th>
				...
				<c:choose>
					<c:when test="${editMode}">
						...
						<tr style="${style}">
							...
							<td><itera:message key="${template.buildingBlockType.name}" /></td>
					...
					<c:otherwise>
						<tr class="link">
							...
							<td onclick="${selectTemplateJavascript}"><a href="javascript:${selectTemplateJavascript}"><itera:message
										key="${template.buildingBlockType.name}" /></a></td>
							<td onclick="${selectTemplateJavascript}"><a href="javascript:${selectTemplateJavascript}"><c:out
										value="${template.description}" /></a></td>
							<td style="vertical-align: middle; white-space: nowrap;">
								<a title="<itera:message key="button.edit" />" href="javascript:${selectTemplateJavascript}">
									<i class="icon-pencil"></i>
								</a>
								<a title="<itera:message key="button.delete" />" href="javascript:confirmDelete(function() {createHiddenField('action','deleteDashboardTemplate'); createHiddenField('customDashboardId','${template.id}'); document.forms[0].submit();});">
									<i class="icon-trash"></i>
								</a>
							</td>
						</tr>
			...
			<tr>
				<td colspan="4">
					<form:select disabled="${editMode}" style="width:auto;" path="selectedBBTypeId">
						<option value="-1">&lt;<itera:message key="customDashboard.template.select.type.popup.title" />&gt;</option>
						<c:forEach var="available" items="${dashboardMemory.bbTAvailableForNewTemplate}">
	 						<form:option value="${available.id}"><itera:message key="${available.name}" /></form:option>
						</c:forEach>
					</form:select>&nbsp;
					<c:choose>
						<c:when test="${editMode}">
							<input disabled="disabled" type="button" id="createDashboardTemplateButton" class="btn" onclick="if ($('#selectedBBTypeId').val() == null || $('#selectedBBTypeId').val() == -1) {showPopupDialog('<itera:message key="customDashboard.template.select.type.popup.title" />', '<itera:message key="customDashboard.template.select.type.popup.content" />');} else {createHiddenField('action', 'createDashboardTemplate'); window.document.forms[0].submit();}" value="<itera:message key="customDashboard.template.create" />" />
						</c:when>
						<c:otherwise>
							<input type="button" id="createDashboardTemplateButton" class="btn" onclick="if ($('#selectedBBTypeId').val() == null || $('#selectedBBTypeId').val() == -1) {showPopupDialog('<itera:message key="customDashboard.template.select.type.popup.title" />', '<itera:message key="customDashboard.template.select.type.popup.content" />');} else {createHiddenField('action', 'createDashboardTemplate'); window.document.forms[0].submit();}" value="<itera:message key="customDashboard.template.create" />" />
						</c:otherwise>
					</c:choose>
				</td>
			</tr>
		...
		<div id="saveDashboardContainer" class="modal hide fade" style="display: none; ">
			...
			<div class="modal-header">
				<a class="close" data-dismiss="modal">×</a>
					<h3><itera:message key="customDashboard.saveTemplate" /></h3>
			</div>
			<div class="modal-body">
			
				<div id="alert_message" class="alert alert-error hide">
					<itera:message key="customDashboard.templateName.error" />
				</div>
			
		    	<div class="control-group">
			    	<label class="control-label" for="titleDashboardInModal" style="width: 100px;">
						<itera:message key="customDashboard.templateName" />
					</label>
					<div class="controls">
			...
			<div class="modal-footer">
				...
				<input type="button" id="saveDashboardButton" class="link btn btn-primary" type="submit" onclick="doSubmit();" value="<itera:message key="button.save" />" />

			</div>
		</div>
		
		<div id="dashboardTemplateEdit" class="module-heading"><itera:message key="customDashboard.template.for.type" />
			<itera:message
		...
		<div class="module-body">
			<div class="btn-group pull-right">
					...
					<a class="btn btn-primary" href="#"
						onclick="$('#saveDashboardContainer').modal('show');" value="<itera:message key="customDashboard.template.create" />"">
				...
				<ul class="nav nav-tabs">
					<li id="tab_TabEditor"
						...
						href="javascript:createHiddenField('action','edit');
										...
										document.forms[0].submit();"><itera:message
