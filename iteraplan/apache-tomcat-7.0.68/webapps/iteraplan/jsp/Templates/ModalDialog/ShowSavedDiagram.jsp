						...
						<table id="ShowSavedQueryTable"
							...
							<thead>
								<tr>
									...
									<th class="col-desc"><itera:message key="global.visualisationType" /></th>
								</tr>
							</thead>
							<tbody>
								<c:forEach items="${savedQueries}" var="savedQuery">
									...
									<tr class="${rowClassString}" style="${rowStyle}">
										<td onclick="${includeDiagramIdOnclickValue}">
											<a href="javascript:<c:out value="${includeDiagramIdOnclickValue}"/>" rel="tooltip" data-original-title="<itera:message key='graphicalReport.insertSavedQuery.tooltip'/>">
											 	<i class="icon-play"></i>
											</a>
										</td>
										<td onclick="${includeDiagramIdOnclickValue}">
