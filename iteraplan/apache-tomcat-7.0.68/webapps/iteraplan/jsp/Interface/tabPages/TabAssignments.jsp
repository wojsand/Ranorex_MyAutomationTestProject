...
<div id="InterfaceToBusinessObjectsModule" class="row-fluid module">
	<div class="module-heading">
		<itera:message key="interface.to.businessObjects" />
	</div>
	<div class="row-fluid">
			...
			<div class="row-fluid">
				<table class="table table-striped table-condensed tableInModule">
								...
								<tbody>
									<c:forEach items="${connected_elements}" var="transport" varStatus="status">
										...
										<tr class="<c:out value="${linkStyle}"/>">
											<td>
												<a class="link" title="<itera:message key="tooltip.remove"/>" onclick="setHiddenField('componentModel.transportModel.elementIdToRemove','<c:out value="${transport.id}"/>');flowAction('update');" >
					...
					<c:if test="${componentMode != 'READ'}">
						<tr>
							<td>
								<a href="#" title="<itera:message key="tooltip.add"/>" onclick="flowAction('update');" class="link" >
									<i class="icon-plus"></i>
								</a>
							</td>
							<td>
