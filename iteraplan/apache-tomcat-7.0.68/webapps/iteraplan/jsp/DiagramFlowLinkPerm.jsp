...
<tilesx:useAttribute name="rest_mapping"/>	

<tilesx:useAttribute name="subflow_base_id"/>

<tilesx:useAttribute name="queryType"/>

<tilesx:useAttribute name="queryType2" ignore="true"/>
...
<c:if test="${permissionGranted == 'true'}" >
	...
	<c:forEach items="${guiContext.flowEntries[this_dialog]}" var="entry">
		<c:if test="${guiContext.activeDialog == entry.key && entry.flowId == subflowId}">
			...
			<c:set var="active_sub_title" scope="request" ><itera:message key="${title_key}"/></c:set>
