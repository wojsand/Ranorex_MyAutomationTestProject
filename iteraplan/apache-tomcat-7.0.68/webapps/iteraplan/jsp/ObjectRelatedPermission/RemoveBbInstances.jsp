...
<div id="RemoveBbInstancesModul" class="row-fluid module">
	<div class="module-heading">
		<itera:message key="objectRelatedPermissions.hasExplicitPermissions"/>
	</div>
	<div class="row-fluid">
					...
					<thead>
						<c:if test="${not empty memBean.dto.associatedBuildingBlocks}">
							<tr>
								...
								<th>
									<itera:message key="global.bbtype"/>
								</th>
								<th>
							...
							<tr>
								<c:if test="${componentMode == 'EDIT'}">
									<td align="left" class="buttonintable top">
										<a class="link" href="#" title="<itera:message key="tooltip.remove"/>" onclick="setHiddenField('selectedBuildingBlockIdToRemove','<c:out value="${bb.id}"/>');flowAction('removeBBP');" >
								...
								<td class="<c:out value="${linkStyle}" /> completereleaseintablelong top" onclick="<c:out value="${linkScript}" />" nowrap="nowrap">
									<itera:htmlLinkToElement isLinked="${linkShow}" link="${link}">
										<itera:message key="${bb.buildingBlockType.typeOfBuildingBlock.value}" />
