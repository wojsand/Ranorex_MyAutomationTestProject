...
<c:if test="${componentMode != 'READ'}">
	...
	<itera:message key="objectRelatedPermissions.selectFurtherInstances"/>:  
	<form:select path="selectedQueryTypeNameDB" onchange="flowAction('changeQueryType');">
		<c:forEach items="${memBean.dto.availableQueryTypes}" var="bbtype">
			<form:option value="${bbtype.typeNameDB}">
				<itera:message key="${bbtype.typeNamePresentationKey}" />
			</form:option>
		</c:forEach>
	</form:select>
	<br />
	...
	<br />
	<input name="requestReport" onclick="flowAction('requestReport');" type="button" class="link btn" value="<itera:message key="button.sendQuery" />"/>
	<input name="resetReport" onclick="flowAction('resetReport');" type="button" class="link btn" value="<itera:message key="button.reset" />"/>
	
    <c:if test="${not empty memBean.results}">
       &nbsp;
       <input name="addInstancePermissions" onclick="flowAction('addPermissions');" type="button" class="link btn" value="<itera:message key="objectRelatedPermissions.addPermissions" />"/>
       <br/>
    </c:if>
