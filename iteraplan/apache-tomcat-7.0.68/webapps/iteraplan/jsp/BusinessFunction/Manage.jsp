...
<c:set var="functionalPermission" scope="request">
	...
	<itera:write name="userContext" property="perms.userHasBbTypeFunctionalPermission(${businessFunction})" escapeXml="false" />
</c:set>
	...
	<c:choose>
		<c:when test="${functionalPermission == true}">
			...
			<tiles:insertTemplate template="/jsp/common/TransactionBar.jsp" >
				...
				<tiles:putAttribute name="service_class" value="BusinessFunctionService" />
			...
			<tiles:insertTemplate template="/jsp/common/ElementNameComponentView.jsp">
				<tiles:putAttribute name="icon_suffix" value="businessfunction" />
			...
			<itera:tabgroup id="ConfigTabs" activeTab="${activeTab}">
				...
				<itera:tab id="TabAssignments" 	textKey="button.assignments"	page="/jsp/BusinessFunction/tabPages/TabAssignments.jsp" showTab="${showTab}"/>
				...
				<c:if test="${componentMode == 'READ' && isHistoryEnabled}">
					<itera:tab id="TabLocalHistory" textKey="global.history"    page="/jsp/BusinessFunction/tabPages/TabHistory.jsp"/>
				</c:if> 
			</itera:tabgroup>
