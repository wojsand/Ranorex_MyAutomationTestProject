...
<itera:message key="massUpdates.standardValueCheckbox.tooltip" var="standardValueSelectedTooltip" />
<itera:message key="massUpdates.lineCheckbox.tooltip" var="lineSelectedTooltip" />
      			...
      			<td colspan="<c:out value="${(memBean.totalNumberOfColumns * 2) + 2}"/>">
              			...
              			<c:choose>
	                		<c:when test="${resultForUpdate.attributeException && !((empty memBean.massUpdatePropertyConfig) && (empty memBean.massUpdateAssociationConfig))}">
		                  		<span class="label label-important">
		                  			...
		                  			<itera:message key="massUpdates.properties.saved"/>
		                  		</span>
	                		</c:when>
	                		<c:otherwise>
		                		<span class="label label-important">
		                			<c:out value="${exception}" escapeXml="false"/>
		        	         		<itera:message key="massUpdates.failed"/>
		        	         	</span>
	                		</c:otherwise>
              			</c:choose>
          			</c:when>
					<c:otherwise>
	            		<span class="label label-success">
	            			<itera:message key="massUpdates.updateWasSucessful" />
	            		</span>
          			</c:otherwise>
        			</c:choose>
      			</td>
