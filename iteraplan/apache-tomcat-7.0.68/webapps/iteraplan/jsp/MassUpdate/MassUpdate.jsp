...
<itera:message key="tooltip.checkBoxColumnToolTip" var="checkAllBoxTitle"/>
	    	...
	    	<th class="col-ico" nowrap="nowrap">
	      	...
	      	<c:choose>
		      	<c:when test="${massUpdateType.typeNamePresentationKey ==  'interface.singular'}">
		      	 	<th class="col-name"><itera:message key="massUpdates.interface" />&nbsp;&nbsp;</th>
			  	</c:when>
		     	<c:otherwise>
		      	 	<th class="col-name"><itera:message key="massUpdates.name" />&nbsp;&nbsp;</th>
			  	</c:otherwise>
		  	</c:choose>
			...
			<c:choose>
				...
				<c:otherwise>
					<c:forEach items="${memBean.massUpdatePropertyConfig}" var="muCfg" varStatus="configIdStatus">
						<c:set var="configId" value="${configIdStatus.index}" />
						<itera:define id="prop_header" name="memBean" property="massUpdatePropertyConfig[${configId}].headerKey" />
			    		<th><itera:message key="${prop_header}" /></th>	
					</c:forEach>
				</c:otherwise>
			</c:choose>
...
<input name="getNewResultSet" onclick="flowAction('getNewResultSet')" type="button" class="link btn" value="<itera:message key="massUpdates.newMassUpdateSelection" />"/>

<c:set var="disabled" value="false" />
<c:if test="${attributeValueCount == 0}">
  <c:set var="disabled" value="true" />
</c:if>
<input name="runMassUpdate" onclick="confirmMassUpdate(function(){flowAction('runMassUpdate');})" type="button" class="link btn" value="<itera:message key="massUpdates.runMassUpdate" />"/>
