...
<c:set var="functionalPermission" scope="request">
	<c:set var="businessObject" value="businessObject.singular" />
	<itera:write name="userContext"
		property="perms.userHasBbTypeFunctionalPermission(${businessObject})" escapeXml="false" />
</c:set>
	...
	<c:choose>
		<c:when test="${functionalPermission == true}">
			...
			<tiles:insertTemplate template="/jsp/common/TransactionBar.jsp" >
				...
				<tiles:putAttribute name="service_class" value="BusinessObjectService" />
			...
			<tiles:insertTemplate template="/jsp/common/ElementNameComponentView.jsp" flush="true">
			    <tiles:putAttribute name="icon_suffix" value="businessobject" />
			...
			<itera:tabgroup id="ConfigTabs" activeTab="${activeTab}">
				<itera:tab id="TabHierarchy" 	textKey="global.hierarchy" 		page="/jsp/BusinessObject/tabPages/TabHierarchy.jsp" showTab="true" />
			  	<itera:tab id="TabAssignments" 	textKey="button.assignments" 	page="/jsp/BusinessObject/tabPages/TabAssignments.jsp" showTab="${showTab}"/>
				...
				<c:if test="${componentMode == 'READ' && isHistoryEnabled}">
					<itera:tab id="TabLocalHistory" textKey="global.history"    page="/jsp/BusinessObject/tabPages/TabHistory.jsp"/>
				</c:if> 
			</itera:tabgroup>
