...
<script type="text/javascript">
    addShortcutForClick("<itera:message key='search.newElement.shortcut'/>","<itera:message key='search.newElement.id'/>");
</script>
	...
	<div class="accordion-group">
		<div class="accordion-heading">
			<a class="accordion-toggle" data-toggle="collapse" data-parent="#contextActionsContainer" href="#contextActionsAccordion"
				...
				<i class="icon-cog"></i>
				<itera:message key="contextMenu.actions" />
		...
		<div id="contextActionsAccordion" class="accordion-body ${show}">
			<div class="accordion-inner">
				...
				<c:when test="${isCommonEaDataDialog}">
					...
					<ul class="nav nav-list">
						<c:if test="${permissionCreateBbType}">
							<li>
								<itera:message var="type_to_create" key="${type}" />
								
								<a id="newElementFromSearch" href="#" onclick="${createNewJavascript}">
									<i class="icon-plus"></i>
									<span rel="tooltip" data-original-title="Hotkey: <img/> <itera:message key='search.newElement.tooltip'/>">
										<itera:message key="contextMenu.actions.createNew.param"><itera:param value="${type_to_create}" /></itera:message>
									</span>
								</a>
							</li>
						</c:if>
						<c:if test="${permissionTabularReporting}">
							<li>
								<a id="tabularReportingLink" href="javascript:changeLocation('${tabReportingUrl}');">
									...
									<itera:message key="global.report.text" />
								</a>
							</li>
						</c:if>
						<c:if test="${permissionMassUpdate && permissionUpdateBbType}">
							<li>
								<a href="javascript:changeLocation('${massUpdateUrl}');">
									...
									<itera:message key="global.mass_updates" />
								</a>
							</li>
						</c:if>
						<c:if test="${guiContext.listableOpenElements}">
							<li>
								<itera:message var="type_to_close" key="${pluraltype}" />
								
								<a href="<c:out value="${closeAllUrl}" escapeXml="false"/>">
									<i class="icon-remove-circle"></i>
									<itera:message key="contextMenu.actions.closeAll.param"><itera:param value="${type_to_close}" /></itera:message>
								</a>
							</li>
						</c:if>
					</ul>
				...
				<c:when test="${isCommonGovernanceDialog}">
					<ul class="nav nav-list">						
						<li>
							<a id="newElementFromSearch" href="#" onclick="${createNewJavascript}">
								<i class="icon-plus"></i>
								<span rel="tooltip" data-original-title="Hotkey: <img/> <itera:message key='search.newElement.tooltip'/>"><itera:message key="button.create" /></span>
							</a>
						</li>
						<c:if test="${guiContext.listableOpenElements}">
							<li>
								<a href="<c:out value="${closeAllUrl}" escapeXml="false"/>">
									<i class="icon-remove-circle"></i>
									<itera:message key="contextMenu.actions.closeAll" />
								</a>
							</li>
						</c:if>
					</ul>	
					...
					<ul class="nav nav-list">						
						<li>
							<a href="javascript:flowAction('close');">
								<i class="icon-repeat"></i>&nbsp;<itera:message key="button.reset" />
							</a>
						</li>
					</ul>
				</c:when>
				...
				<c:otherwise>
					<itera:message key="global.contextMenu.actions.empty" />
				</c:otherwise>
				</c:choose>
			</div>
		</div>
