...
(function($) {
	$.widget("ui.combobox", $.ui.autocomplete, {
		options : {
			...
			onlyAvailableForInterfaces : false,
		...
		_setupMenuItem : function(items, source) {
			...
			for ( var i = 0; i < itemsChunk.length; i++) {
				$(itemsChunk[i]).data("item.autocomplete", sourceChunk[i]).mouseenter(function(event) {
					...
					temp = self.element.context.value;

					// because this next line empties the current value on the first run
					self.menu.deactivate();

					// and now we fill the input field with stored current value
					self.element.val(temp);
				});
			}
			if (items.length > 0) {
				setTimeout(function() {
		...
		_fetch : function(processValues) {
			...
			$.ajax({
				...
				data : {
					...
					onlyAvailableForInterfaces : this.options.onlyAvailableForInterfaces,
					escapeHtml: true
				},
				dataType : "json",
		...
		_selectInit : function() {
			...
			if ((select.attr("ownbbtype") == 'informationsysteminterface') && (this.options.bbtype == 'technicalcomponent')) {
				this.options.onlyAvailableForInterfaces = true;
			}
