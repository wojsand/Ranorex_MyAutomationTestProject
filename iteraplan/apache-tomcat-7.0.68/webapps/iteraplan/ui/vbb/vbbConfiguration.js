function Configuration(vbbName, submitButton) {
	...
	function _refreshDiscreteColoringOptions(container, availableColors, orderedLiterals, selectedColors, discreteColorMappingHidden) {
		...
		$.each(orderedLiterals, function(index, literal) {
			discreteColorContent = discreteColorContent + '<div  class="control-group"><div class="controls">';
			discreteColorContent = discreteColorContent +  _createColorSelect(availableColors, selectedColors[index], index);
	...
	function _addSubstantialTypeCandidate(substantialType, candidateValue, parentValue) {
		...
		var html = '<li id="' + (isChildCandidate ? ':' + parentValue.id :'') + ':' + escapeHTML(candidateValue.id) + '" class="' + substantialType.id + 'Draggable draggable' + (isChildCandidate ? ' childDraggable' : '');
		html += (candidateValue.root ? ' rootDraggable' : '') + '" style="font-size:' + ((5 + candidateValue.prio) * 20) + '%">';
		html += '<span class="symbol">' + (candidateValue.children.length > 1 ? '+' : '') + '</span> <span class="text">' + escapeHTML(candidateValue.name) + '</span></li>';
	...
	function _addFeatureCandidates(feature, candidates) {
		...
		$.each(candidates, function(index, candidate) {
			...
			var html = '<option class="listItem" value="' + candidate.id + '"'+selected+'>' + escapeHTML(candidate.name) + '</option>';
