...
function confirmDeleteBuildingBlocks(callback) {
  showConfirmDialog("<itera:message key='global.confirmDelete' />",
	  				"<itera:message key='global.confirmDeleteBuildingBlocks' />",
	                callback)
}

function confirmDelete(callback) {
  showConfirmDialog("<itera:message key='global.confirmDelete' />",
					"<itera:message key='global.confirmDelete' />",
					callback);
}

function confirmMassUpdate(callback) {
  showConfirmDialog("<itera:message key='massUpdates.runMassUpdate' />",
					"<itera:message key='global.confirmMassUpdate' />",
					callback);
}

function confirmMassenDelete(callback) {
  showConfirmDialog("<itera:message key='massUpdates.requestMassDelete' />",
					"<itera:message key='global.confirmMassenDelete' />",
					callback);
}

function confirmLogout(callback) {
  showConfirmDialog("<itera:message key='menu.logout' />",
					"<itera:message key='global.confirmLogout' />",
					callback);
}

function confirmEdited() {
  showConfirmDialog('<itera:message key='navigation.reset' />',
					'<itera:message key='global.confirmResetWithOpenTransactions' />',
					'<c:url value="/show/restart" />');
}

function msgOkCancel(callback) {
  showConfirmDialog("<itera:message key='button.cancel' />",
					"<itera:message key='global.confirm' />",
					callback);
}
...
function addTransactionBarShortcuts()
{
    ...
    <c:forTokens items="${transactions}" delims=", " var="transaction">
       addShortcutForClick("<itera:message key='transaction.${transaction}.shortcut'/>","<itera:message key='transaction.${transaction}.id'/>");
	</c:forTokens>
}
...
function addTabShortcuts()
{
   <c:set var="tabNumbers">first, second, third, fourth, fifth</c:set>
    ...
    <c:forTokens items="${tabNumbers}" delims=", " var="tabNumber">
	   shortcut.add("<itera:message key="tab.${tabNumber}.shortcut"/>", function() {
	      newSwitchTab("<itera:message key='tabgroup.tag'/>","<itera:message key='tab.${tabNumber}.class'/>");
       }, { 'disable_in_input':true } );
    </c:forTokens>

    shortcut.add("Shift+Left", function() {
	   rotateTabs("<itera:message key='tabgroup.tag'/>", -1);
    }, { 'disable_in_input':true } );    
...
{
   addShortcutForClick("<itera:message key='jumpToInterface.shortcut'/>","<itera:message key='jumpToInterface.id'/>");
}
...
function addSearchShortcuts()
{
     ...
     <c:forTokens items="${searchActions}" delims=", " var="searchAction">
        addShortcutForClick("<itera:message key="search.${searchAction}.shortcut"/>","<itera:message key='search.${searchAction}.id'/>");
     </c:forTokens>
}
...
function addMiscShortcuts()
{
   ...
   <c:forTokens items="${miscButtons}" delims=", " var="miscButton">
      addShortcutForClick("<itera:message key="button.${miscButton}.shortcut"/>","<itera:message key='button.${miscButton}.id'/>");
   </c:forTokens>
}
...
function loadSavedQueries(reportType, destinationId, flow_url) {
	...
	var liLoading = $('<li><itera:message key="global.loading"/></li>').appendTo('#' + destinationId);
		...
		success :  function(data){
			...
			if(dropdownList != null){
				...
				for (var i = 0; i < data.length; i++) {
					...
					var escapedQueryName = $('<div/>').text(data[i].name).html(); 
					
					liElement.append($('<a href=\"javascript:preLoadSavedQuery(' + params + ');\" >' + escapedQueryName + '</a>'));
